...
type Route = "pairing" | "home" | "chat" | "settings" | "activity" | "notifications" | "personalization";

function getRoute(): Route {
  // '#/notifications' must be tested BEFORE '#/notif-overlay' would match via
  // prefix — but notif-overlay is a separate window that never mounts App, so
  // only the main window's hash reaches here. Still, keep the prefix distinct.
  ...
  if (window.location.hash.startsWith("#/personalization")) return "personalization";
...
export default function App() {
  ...
  // Outbound: poll my own foreground app every ~4s. Must run before
  // buildSnapshot / snapshotRef — buildSnapshot reads myActivity.
  const { activity: myActivity, activityPath: myActivityPath } = useActivity(
    connected === "open",
    settings.share_activity,
  );
  ...
  // Latest widget snapshot, kept in a ref so the `widget-ready` handshake
  // listener (mounted once, deps []) can re-emit the CURRENT snapshot instead
  // of the stale one captured at first render — without re-binding the listener
  // on every state change. Mirrors the partnerNameRef pattern.
  const snapshotRef = useRef<WidgetSnapshot>(buildSnapshot());
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (existing?.partner_id && existing?.device_secret) {
        ...
        // App-lifetime chat receiver: persist + route received messages even
        // while ChatScreen is unmounted (otherwise a message arriving on Home
        // was notified but never saved → "não chegou a mensagem"). Idempotent.
        messages.start(id.partner_id!, id);
        if (getRoute() === "pairing") window.location.hash = "#/home";
      } else {
        // Not paired yet: ensure a device id exists, stay on pairing.
        const fresh = await ensureIdentity(s.relay_url);
        setId(fresh);
      }
      } catch (e) {
      ...
      if (e.type === "activity") {
        ...
        try {
          ...
          if (changed) {
            await saveIdentity(next);
            setId(next);
            // Mirror the freshly refreshed name/avatar to the receive side so
            // harbor_notify notification cards show the current identity. The
            // partner's pubkey isn't pushed here (it changes via /partner, not
            // profile_update), so we don't touch it in this handler.
            messages.setPartnerIdentity({
              partner_name: next.partner_name,
              partner_avatar: next.partner_avatar,
            });
          }
        } catch {
  ...
  // Reconcile the OS autostart registration with the saved setting. The Settings
  // UI registers/unregisters immediately on toggle (no Salvar), but this is the
  // safety net that keeps the Windows Run key in sync across launches: if the
  // saved setting is on but the OS registration was lost (a portable build moved
  // paths, an uninstall removed the Run key, a prior enable() silently failed),
  // this re-registers. Conversely it unregisters if the setting is off but the
  // registration lingers. The plugin's enable()/disable() are idempotent, so
  // re-applying the saved value on every boot is safe and cheap. Wrapped so a
  // plugin-not-available target (capability missing / init failed) never rejects
  // to the console — the Settings toggle surfaces that case honestly instead.
  useEffect(() => {
    setAutostart(settings.autostart_enabled).catch(() => {});
  }, [settings.autostart_enabled]);

  // Apply the personalization palette whenever it changes, AND re-apply when the
  // resolved dark/light mode changes so Sistema flips the right per-mode palette
  // live (the effect re-runs because `resolvedDark()` is derived from
  // settings.theme + the OS). For "system" we attach the OS listener and re-apply
  // on change too — the palette is per-mode, so a live OS theme flip must re-paint
  // the matching palette, not the one captured at mount. Cheap: one setProperty
  // per token.
  useEffect(() => {
    const dark = resolvedDark();
    applyCustomization(customizationOf(settings), dark);
    if (settings.theme !== "system") return () => {};
    const mq = window.matchMedia("(prefers-color-scheme: dark)");
    const handler = () => applyCustomization(customizationOf(settings), mq.matches);
    mq.addEventListener("change", handler);
    return () => mq.removeEventListener("change", handler);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [settings.customization, settings.theme]);
  ...
  function buildSnapshot(): WidgetSnapshot {
    return {
      ...
      dark: resolvedDark(),
      customization: customizationOf(settings),
    };
  }
  ...
  useEffect(() => {
    ...
    myActivity,
    voiceState,
    // Personalização: a palette/preset/mascot change re-paints the widget (it
    // has no store; its custom colors arrive only via this snapshot). Resolved
    // dark is derived from settings.theme, already in the deps.
  ...
  // Widget-ready handshake: the widget emits `harbor-widget-ready` once its
  // `harbor-widget-state` listener is armed (WidgetScreen mount). Re-emit the
  // current snapshot when we hear it — closes the boot race where the main
  // window's first `emitNow()` (right after `openWidget()` created the widget)
  // fired before the freshly-created second realm had subscribed, stranding the
  // widget on the placeholder until a manual disable→enable re-opened it. The
  // snapshot is read fresh via `snapshotRef` (kept current on every render) so a
  // once-mounted listener never emits a stale snapshot. Best-effort (the emitter
  // swallows errors); a missed ready is harmless — the next reconciler emit
  // still arrives.
  useEffect(() => {
    let off: (() => void) | undefined;
    listen(WIDGET_READY_EVENT, () => {
      void emitWidgetState(snapshotRef.current);
    }).then((un) => {
      off = un;
    });
    return () => off?.();
  ...
  useEffect(() => {
    ...
    return (
      <div className="window-main flex h-screen flex-col items-center justify-center px-6 text-center">
        ...
        <p className="mt-3 max-w-sm text-sm leading-relaxed" style={{ color: "var(--color-harbor-sec)" }}>
          {bootError}
        </p>
      </div>
    );
...
/** Shown while identity/settings load after UpdateGate releases the app. */
