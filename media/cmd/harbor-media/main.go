...
// sides of a call: connection-state events, relayed ICE, one Opus track that
// the voice pipeline feeds, and one VP8 track the share pipeline feeds. The
// video track is negotiated with the call itself, so a share start writes
// frames onto an existing sender and never renegotiates the audio track away.
//
// stunServers arrive from the core (same host/port as the control plane over
// UDP); when present they add server-reflexive candidates so direct paths
// survive NAT. Empty keeps the historical host-candidates-only behavior.
func (s *service) newCallPeerConnection(callID string, stunServers []string, turnServers []turnICEServer) (*webrtc.PeerConnection, *callTracks, error) {
	pc, err := webrtc.NewPeerConnection(iceConfigurationWithTURN(stunServers, turnServers))
	if err != nil {
		return nil, nil, err
	}
...
// iceConfiguration maps validated STUN server URLs into a WebRTC
// configuration. Malformed entries are dropped, never sent to Pion: a STUN
// URL must look like stun:host:port (bracketed IPv6 accepted) or the whole
// list degrades to empty — host candidates still gather, srflx just will
// not. An empty list keeps host-candidates-only behavior exactly.
func iceConfiguration(stunServers []string) webrtc.Configuration {
	return iceConfigurationWithTURN(stunServers, nil)
}

func iceConfigurationWithTURN(stunServers []string, turnServers []turnICEServer) webrtc.Configuration {
	...
	for _, turn := range turnServers {
		...
		urls := make([]string, 0, len(turn.URLs))
		for _, raw := range turn.URLs {
			if validTURNURL(raw) {
		...
		if len(urls) > 0 {
			servers = append(servers, webrtc.ICEServer{
				URLs: urls, Username: turn.Username, Credential: turn.Credential,
			})
		}
	}
	if len(servers) == 0 {
...
func validTURNURL(raw string) bool {
	return strings.HasPrefix(raw, "turn:") && len(raw) > len("turn:")
}

// validSTUNURL accepts stun:host:port with a nonempty host and a nonzero
// numeric port, tolerating bracketed IPv6 literals (cut at `]:`, since bare
// colons inside brackets are not separators). Pion does the real
// resolution; this gate only keeps obvious garbage (and non-stun schemes)
// from ever reaching it.
func validSTUNURL(raw string) bool {
	...
	if strings.HasPrefix(rest, "[") {
		...
		host, port = rest[:end+1], rest[end+2:]
		if len(host) < 3 {
			return false
		}
	} else {
...
func (s *service) startCall(request envelope) envelope {
	var payload struct {
		...
		TurnServers     []turnICEServer `json:"turn_servers"`
	}
	...
	pc, tracks, err := s.newCallPeerConnection(payload.CallID, payload.StunServers, payload.TurnServers)
	if err != nil {
