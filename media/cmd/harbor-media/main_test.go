...
func TestValidSTUNURLAcceptsHostPortAndBrackets(t *testing.T) {
	for _, raw := range []string{
		"stun:192.0.2.10:9091",
		"stun:[2001:db8::1]:9091",
		"stun:harbor.example:443",
		"stun:localhost:3478",
	} {
		if !validSTUNURL(raw) {
			t.Errorf("valid STUN URL rejected: %q", raw)
		}
	}
	for _, raw := range []string{
		"",
		"stun:",
		"turn:192.0.2.10:9091",
		"stun:192.0.2.10",
		"stun::9091",
		"stun:192.0.2.10:0",
		"stun:192.0.2.10:99999",
		"stun:192.0.2.10:http",
		"stun:2001:db8::1:9091",
		"stun:[2001:db8::1]",
		"stun:[]:9091",
		" stun:192.0.2.10:9091",
...
func TestICEConfigurationMapsValidatedServers(t *testing.T) {
	config := iceConfiguration([]string{"stun:192.0.2.10:9091", "stun:[2001:db8::1]:443"})
	if len(config.ICEServers) != 2 {
		t.Fatalf("expected 2 ICE servers, got %#v", config.ICEServers)
	}
	if len(config.ICEServers[0].URLs) != 1 || config.ICEServers[0].URLs[0] != "stun:192.0.2.10:9091" {
		t.Fatalf("first server mangled: %#v", config.ICEServers[0])
	}
	if len(iceConfiguration(nil).ICEServers) != 0 {
		t.Fatal("nil servers must keep host-candidates-only behavior")
	}
	if len(iceConfiguration([]string{"turn:x:1", "", "stun:ok.example:3478"}).ICEServers) != 1 {
		t.Fatal("garbage entries must drop, leaving only the valid one")
	}
}

func TestICEConfigurationMapsTURNCredentials(t *testing.T) {
	config := iceConfigurationWithTURN(
		[]string{"stun:192.0.2.10:9091"},
		[]turnICEServer{
			{URLs: []string{"turn:192.0.2.10:9091?transport=udp"}, Username: "device:123", Credential: "secret"},
			{URLs: []string{"turn:bad"}, Username: "", Credential: "ignored"},
		},
	)
	if len(config.ICEServers) != 2 {
		t.Fatalf("expected STUN and TURN servers, got %#v", config.ICEServers)
	}
	turn := config.ICEServers[1]
	if turn.Username != "device:123" || turn.Credential != "secret" {
		t.Fatalf("TURN credentials were not mapped: %#v", turn)
	}
}

// startTestSTUNServer answers binding requests with the sender's observed
// address: a minimal stand-in proving Pion actually consults the configured
// server (the production server speaks the same RFC on its UDP socket).
func startTestSTUNServer(t *testing.T) (string, func()) {
	t.Helper()
	conn, err := net.ListenPacket("udp4", "127.0.0.1:0")
	if err != nil {
		t.Fatalf("cannot bind test STUN socket: %v", err)
	}
	...
	go func() {
		...
		for {
			...
			request.Raw = append([]byte(nil), buffer[:n]...)
			...
			xor := stun.XORMappedAddress{IP: udp.IP, Port: udp.Port}
			if err := xor.AddTo(response); err != nil {
				continue
			}
			response.Encode()
			_, _ = conn.WriteTo(response.Raw, src)
		}
	}()
...
func TestPeerConnectionGathersServerReflexive(t *testing.T) {
	stunAddr, stopSTUN := startTestSTUNServer(t)
	defer stopSTUN()
	host, port, err := net.SplitHostPort(stunAddr)
	if err != nil {
		t.Fatalf("test STUN address unusable: %v", err)
	}
	pc, err := webrtc.NewPeerConnection(iceConfiguration([]string{"stun:" + host + ":" + port}))
	if err != nil {
		t.Fatalf("peer connection with STUN config failed: %v", err)
	}
	defer func() { _ = pc.Close() }()
	if _, err := pc.CreateDataChannel("probe", nil); err != nil {
		t.Fatalf("data channel for gathering failed: %v", err)
	}
	srflx := make(chan string, 8)
	pc.OnICECandidate(func(candidate *webrtc.ICECandidate) {
		if candidate == nil {
			return
		}
		init := candidate.ToJSON()
		if strings.Contains(init.Candidate, " srflx ") || strings.Contains(init.Candidate, " typ srflx") {
			srflx <- init.Candidate
		}
	})
	...
	if err := pc.SetLocalDescription(offer); err != nil {
		t.Fatalf("local description failed: %v", err)
	}
	select {
	case candidate := <-srflx:
		if !strings.Contains(candidate, "127.0.0.1") {
			t.Fatalf("srflx candidate does not carry the loopback-mapped address: %q", candidate)
		}
	case <-time.After(10 * time.Second):
		t.Fatal("no server-reflexive candidate gathered through the STUN server")
	}
}
