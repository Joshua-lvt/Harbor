...
const NETWORK_CONTROL_MESSAGE_TYPES: &[&str] = &[
    ...
    "session.signal_poll",
    "contacts.list",
    // Rendezvous: the server echoes the transport endpoint it observed for
    // the caller. No peer facts travel here; peer-side discovery composes
    // this with invites on the client.
    "endpoint.self",
    // Relay fallback: opaque, size-bounded data frames between paired
    // devices through their existing outbound connections. The server
    // routes by session id and never interprets payloads; end-to-end
    // encryption lives on the clients (Phase 1.4). `relay.error` is
    // server-originated (carried in poll answers); clients must not send it.
    // `relay.flow` is reserved for sender-side pause signaling (Phase 2+).
    "relay.open",
    "relay.accept",
    "relay.data",
    "relay.poll",
    ...
    "relay.error",
    "relay.flow",
    // TURN media fallback: an authenticated device mints short-lived
    // Allocate credentials here; the UDP socket never sees the control
    // plane. Password travels once, over the pinned TLS control channel.
    "turn.credentials",
];
...
mod tests {
    ...
    #[test]
    fn network_diagnostics_is_local_only() {
        let diagnostics =
            Envelope::request("network.diagnostics", json!({}), "2026-08-31T20:00:00Z");
        assert_eq!(diagnostics.validate(), Ok(()));
        assert_eq!(
            diagnostics.validate_network(),
            Err(ValidationError::ForbiddenMessageType(
    ...
    #[test]
    fn relay_fallback_types_are_signable_control_plane_messages() {
        ...
        for message_type in [
            ...
            assert_eq!(frame.validate_network(), Ok(()));
            assert!(AuthenticatedEnvelope::sign(Uuid::new_v4(), frame, &signing_key).is_ok());
        }
    }

    #[test]
    fn turn_credentials_type_is_signable_control_plane_message() {
        let signing_key = SigningKey::from_bytes(&[12; 32]);
        let frame = Envelope::request("turn.credentials", json!({}), "2026-08-31T20:00:00Z");
        assert_eq!(frame.validate_network(), Ok(()));
        assert!(AuthenticatedEnvelope::sign(Uuid::new_v4(), frame, &signing_key).is_ok());
    }
