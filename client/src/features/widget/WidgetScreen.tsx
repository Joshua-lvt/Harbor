...
import { emitWidgetClosed, WIDGET_READY_EVENT, type WidgetSnapshot } from "../../services/widget";
...
const INITIAL: WidgetSnapshot = {
  ...
  // Ocean defaults until the first snapshot arrives — matches the main window's
  // pre-emit palette so the widget doesn't flash the wrong colors before its
  // first state event (custom colors arrive only via the snapshot).
...
export default function WidgetScreen() {
  ...
  // Apply the personalization palette from the snapshot — the widget is a
  // separate JS realm, so the main window's inline `:root` overrides never cross
  // over; the snapshot carries the resolved palette + bg/mascot flags and we
  // paint them here, mirroring how `dark` arrives via the snapshot too.
  useEffect(() => {
    applyCustomization(snap.customization, snap.dark);
  }, [snap.customization, snap.dark]);

  useEffect(() => {
    let off: (() => void) | undefined;
    listen<WidgetSnapshot>("harbor-widget-state", (e) => setSnap(e.payload)).then((un) => {
      off = un;
      // Handshake: the listener is now armed — ask the main window to re-emit
      // its snapshot. This fixes the boot race where the main window's first
      // `emitNow()` (right after `openWidget()` created this window) fired
      // before this freshly-created second realm had subscribed, stranding the
      // widget on the placeholder until a manual toggle re-opened it. Emitting
      // here means the widget gets its initial state whether it mounts before
      // or after the first emit. Best-effort (the emit helper swallows errors).
      void emit(WIDGET_READY_EVENT);
    });
    return () => off?.();
  }, []);
