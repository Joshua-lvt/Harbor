 ...
 * name, my own presence dot), and the vertical navigation. Real routes use
 * the existing hash routing (Início/Chat/Configurações); Atividade is NOT a
 * route here (per the redesign decision — it's a state indicator that reflects
 * live partner activity rather than opening a dedicated screen). The voice call
 * has no nav entry anymore — it's always on and lives as the `CallStrip` at the
 * top of the main column, not a separate screen. The footer reuses
 * `MyActivityBadge` so the local "what I'm doing right now" stays accurate
 * without a second source.
 */
import { Home, MessageCircle, Gamepad2, Settings } from "lucide-react";
...
const NAV: NavItem[] = [
  ...
  // Feature 3: Atividade is now a real route (the chronological history
  // screen), not a disabled pip. The nav highlight uses the hash prefix.
    ...
    <aside className="flex h-full w-60 shrink-0 flex-col bg-harbor-sidebar/95 text-white/90 backdrop-blur-md">
      ...
      <nav className="mt-1 flex flex-1 flex-col gap-1 px-3">
        {NAV.map((item) => {
          ...
          // Every NAV item is a route now (Atividade included — Feature 3).
          const active = !!item.route && currentHash.startsWith(item.route.slice(0, 5));
