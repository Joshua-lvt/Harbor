...
import { setAutostart, isAutostartEnabled } from "../../services/autostart";
...
export default function SettingsScreen({
  ...
  async function saveAll() {
    const next: Settings = { ...s, relay_url: relayUrl, autostart_enabled: autostart };
    ...
    try {
      ...
      // Plugin not registered on this target — ignore.
    }
    setSaved("Salvo!");
  ...
  /** Apply a theme choice immediately (so the user sees it live), persist it,
   *  and lift it to App state (App's theme effect re-runs on `settings.theme`
   *  and re-attaches/​detaches the OS listener for "system"). */
  async function setTheme(theme: ThemeOption) {
    applyTheme(theme);
    const next: Settings = { ...s, theme };
    setS(next);
    await saveSettings(next);
    onSettings(next);
  }
  ...
  return (
    <div className="window-main h-screen flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-5">
        ...
        <section className="flex flex-col gap-2">
          <p className="text-xs font-medium text-harbor-deep/70">Aparência</p>
          ...
          <p className="text-[11px] text-harbor-ink/50">
            "Sistema" segue o tema do Windows — muda sozinho quando você alterna. Sua escolha vale sem reiniciar.
          </p>
        </section>
        ...
        <section className="flex items-center justify-between rounded-xl bg-harbor-surface/70 px-3 py-2">
          ...
          <input
            type="checkbox"
            checked={autostart}
            onChange={(e) => setAutostartState(e.target.checked)}
            className="h-4 w-4 accent-harbor-deep"
          />
        </section>
