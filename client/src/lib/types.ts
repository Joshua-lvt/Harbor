...
export interface Settings {
  ...
  relay_url: string;
  /** "Iniciar com o Windows" — launches Harbor at OS boot (Feature: autostart).
   *  ON BY DEFAULT: a couple-presence app is most useful when it's always
   *  running, so new installs and legacy stores missing the key both default
   *  on (`loadSettings` merges `DEFAULT_SETTINGS` over the persisted store). An
   *  install that explicitly turned it OFF keeps `true`→`false` persisted, so
   *  the stored `false` is respected — we never force it back on. The OS Run
   *  key is reconciled with this value every launch (App.tsx effect), so the
   *  registration heals itself if lost. See services/autostart.ts. */
  ...
  /** User-recolorable palette + a couple of extras (feature: Personalização).
   *  Light + dark keep separate palettes; applied live to `:root` inline by
   *  `lib/customization.ts:applyCustomization` (App.tsx effect). Migration is
   *  inert — an old settings.json without this key merges to the ocean defaults
   *  via `resolveCustomization`. */
