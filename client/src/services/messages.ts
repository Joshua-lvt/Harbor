...
class MessageStore {
  ...
  /** True while the ChatScreen is mounted (the user is actively viewing this
   *  conversation). Suppresses the redundant "Nova mensagem recebida" OS toast —
   *  a message the user just saw land inline as a bubble should not also ding
   *  the system tray. Flipped by ChatScreen via setChatActive on mount/unmount. */
   ...
   *  viewing the conversation). When true, received messages skip the OS
   *  "Nova mensagem recebida" toast — they already landed inline as bubbles.
   *  Must stay accurate across route changes, so ChatScreen toggles it in an
   *  effect (set true on mount, restore false on cleanup). */
