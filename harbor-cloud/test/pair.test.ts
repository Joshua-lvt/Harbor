...
describe("transient forwards — voice / activity / typing (#11–14)", () => {
  it("forwards each voice_signal kind and never buffers (partner online)", async () => {
    ...
    // The client sends voice_signal.data as real WebRTC objects (an SDP
    // description for offer/answer, a candidate object for ice) — NOT strings.
    // The relay forwards them verbatim (audio is P2P; signaling is opaque to
    // it). A prior check `typeof data !== "string"` rejected every object the
    // client sent → the offer never reached the partner → infinite reconnect.
    const sdpOffer = { type: "offer", sdp: "v=0\r\no=- 1 1 IN IP4 0\r\ns=-\r\n" };
    const sdpAnswer = { type: "answer", sdp: "v=0\r\no=- 2 2 IN IP4 0\r\ns=-\r\n" };
    const iceCandidate = { candidate: "candidate:1 1 udp 1 1.2.3.4 1 typ host", sdpMid: "0", sdpMLineIndex: 0 };
    const cases: Array<{ kind: "offer" | "answer" | "ice"; data: unknown }> = [
      { kind: "offer", data: sdpOffer },
      { kind: "answer", data: sdpAnswer },
      { kind: "ice", data: iceCandidate },
    ];
    for (const { kind, data } of cases) {
      wa.send(JSON.stringify({ type: "voice_signal", kind, data }));
      const got = await recvJson(wb, 1000);
      expect(got.type).toBe("voice_signal");
      expect(got.kind).toBe(kind);
      expect(got.device_id).toBe(aid);
      // Forwarded untouched — the relay must not reshape inline SDP/candidate
      // objects (deep-equal comparison: it must pass through field-for-field).
