...
describe("validateClientMessage — voice_signal", () => {
  it("accepts offer/answer/ice and forwards the data payload verbatim (signaling is opaque to the relay)", () => {
    // The client sends real WebRTC objects, not strings: an SDP description
    // (offer/answer) or a candidate object (ice). The relay forwards them
    // untouched (audio is P2P; signaling is never inspected). A prior
    // `typeof data === "string"` check dropped every object → the offer never
    // reached the partner → infinite reconnect loop (voice.ts re-offers 8s).
    const sdp = { type: "offer", sdp: "v=0\r\no=- 1 1 IN IP4 0\r\ns=-\r\n" };
    for (const kind of ["offer", "answer", "ice"] as const) {
      const r = ok({ type: "voice_signal", kind, data: sdp });
      expect(r.ok).toBe(true);
      if (r.ok) expect(r.msg).toEqual({ type: "voice_signal", kind, data: sdp });
    }
    // A plain string still works too (forwarded verbatim either way).
    expect(isOk({ type: "voice_signal", kind: "ice", data: "candidate:..." })).toBe(true);
  });
  ...
  it("rejects a missing data payload", () => {
    expect(isOk({ type: "voice_signal", kind: "ice" })).toBe(false);
    expect(errReason({ type: "voice_signal", kind: "ice" })).toBe("bad_signal_data");
  });
});
