...
QtObject {
    ...
    readonly property var incomingRequestSequence: [
        { harborId: "harbor-1a2b3c4d", name: "Morgan", initials: "MO" },
        { harborId: "harbor-5e6f7890", name: "Avery", initials: "AV" },
        { harborId: "harbor-ab12cd34", name: "Taylor", initials: "TA" }
    ]
    ...
    // Pairing and incoming-request state. Same contract as the production
    // bridge: Harbor-ID based (home, connecting, success, error, incoming).
    // Demo Harbor IDs are fixtures, never real identities.
    readonly property string demoOwnHarborId: "harbor-dead0001"
    readonly property string ownHarborId: demoOwnHarborId
    property string pairingMode: "home" // home, connecting, success, error, incoming
    property string enteredHarborId: ""
    readonly property string harborIdFieldState: {
        var value = String(enteredHarborId || "")
        if (value.length === 0)
            return "empty"
        return isValidHarborId(value) ? "valid" : "invalid"
    }
    property bool connecting: false
    property string pairingErrorKey: ""
    property var pairingErrorParams: ({})
    property string pendingPartnerHarborId: "harbor-dead0002"
    ...
    function openPairing(initialMode) {
        ...
        enteredHarborId = ""
        connecting = false
        pendingPartnerHarborId = "harbor-dead0002"
        AppState.pairingVisible = true
        setPairingMode(initialMode || "home")
    }
    ...
    function setPairingMode(mode) {
        // Legacy mode names from older previews/tests land on home.
        var values = ["home", "connecting", "success", "error", "incoming"]
        pairingMode = values.indexOf(mode) >= 0 ? mode : "home"
        return pairingMode
    }

    /// Canonical Harbor-ID check, mirroring the core and server gates: full
    /// string match only, no trimming, no numeric conversion.
    ...
    function connectWithHarborId(value) {
        ...
        enteredHarborId = String(value || "")
        pairingErrorParams = ({})
        if (!isValidHarborId(harborId)) {
            pairingErrorKey = ""
            return false
        }
        enteredHarborId = harborId
        // Deterministic demo outcomes: the all-zero ID previews a decline,
        // anything else valid previews an approval after the delay.
        if (harborId === "harbor-00000000") {
            pairingErrorKey = "pairing.error.declined"
            pairingMode = "error"
            return true
        }
        pairingErrorKey = ""
        pendingPartnerHarborId = harborId
        connecting = true
    ...
    function retryConnect() {
        connecting = false
        return connectWithHarborId(enteredHarborId)
    }

    function copyHarborId() {
        return mockCopy(demoOwnHarborId, "harborId")
    }
    ...
    function completePairing(partnerHarborId) {
        ...
        pairingRequestRunning = false
        connecting = false
        pendingPartnerHarborId = String(partnerHarborId || pendingPartnerHarborId
                                        || "harbor-dead0002")
        pairingMode = "success"
        AppState.updatePartnerProfile({
            name: pendingPartnerHarborId,
            initials: AppState.initialsFor(pendingPartnerHarborId),
            presence: "online"
        })
        _applyConnectionState("connected")
        queueLocalizedToast("online", "toast.harborReady.title", {},
                            "toast.harborReady.description", { name: pendingPartnerHarborId })
        pairingCompleted(pendingPartnerHarborId)
        return pendingPartnerHarborId
    }
    ...
    function showIncomingRequest(request) {
        ...
        var harborId = String(source.harborId || "harbor-1a2b3c4d")
        incomingRequest = {
            harborId: harborId,
            name: String(source.name || harborId),
            initials: String(source.initials || AppState.initialsFor(harborId)),
    ...
    function acceptIncomingRequest() {
        if (pairingMode !== "incoming")
            return false
        var harborId = incomingRequest.harborId
        incomingRequest = ({ harborId: "", name: "", pairingId: "" })
        completePairing(harborId)
        return true
    }
    ...
    function declineIncomingRequest() {
        ...
        incomingRequest = ({ harborId: "", name: "", pairingId: "" })
        closePairing()
        incomingPairingDeclined(harborId)
        return true
    }
    ...
    property Timer pairingRequestTimer: Timer {
        onTriggered: controller.completePairing(controller.pendingPartnerHarborId)
    }
