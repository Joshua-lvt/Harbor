...
TestCase {
    ...
    function test_pairingPagesRenderAcrossModes() {
        ...
        try {
            var modes = ["home", "connecting", "error", "incoming", "success"]
            ...
            // The controller, not the view, owns the pairing state.
    ...
    function test_onboardingSingleScreenPairing() {
        ...
        try {
            // Single pairing screen: Harbor ID visible, peer Harbor-ID
            // entry, connect action, and an explicit session-scoped bypass.
            verify(view.selfHarborId !== undefined)
            compare(view.peerHarborId, "")
            compare(view.uiState, "INITIAL")

            // Malformed IDs are refused honestly without touching the provider.
            view.peerHarborId = "123"
            verify(!view.connectWithHarborId())
    ...
    // Empty input keeps Connect disabled; typing a malformed Harbor ID and
    // pressing Enter surfaces an honest inline notice with no request sent.
    function test_onboardingAdvancesWithKeyboardActivation() {
        ...
        try {
            ...
            verify(!primary.enabled, "Connect stays disabled for empty input")

            var field = _findFirst(view, "onboardingPeerHarborInput")
            verify(field !== null, "Harbor-ID field should be reachable")
            field.forceInputFocus()
            // Programmatic sets flow through the view property so the
            // binding (not just the pixels) carries the malformed value.
            view.peerHarborId = "123"
            KeyTest.click(field, Qt.Key_Return)
            verify(view.noticeMessage.length > 0)
            verify(AppState.onboardingVisible)

            view.peerHarborId = "harbor-d31846b8"
