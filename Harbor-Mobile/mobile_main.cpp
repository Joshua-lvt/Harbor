...
int main(int argc, char *argv[])
{
    ...
    // Oracle-default release builds (HARBOR_ORACLE_ADDRESS at configure
    // time): the QML host prefers this over the K11 literal for fresh
    // installs. Empty unless baked in — the address never lives in QML.
    engine.rootContext()->setContextProperty(
        QStringLiteral("oracleDefaultAddress"),
        QStringLiteral(HARBOR_ORACLE_ADDRESS_BUILTIN).trimmed());
#else
    engine.rootContext()->setContextProperty(QStringLiteral("oracleDefaultAddress"), QString());
#endif
