...
- Default endpoint: `[2804:d59:8777:ad00:3a30:f9ff:fe3e:de81]:9091`
  (public IPv6 path, the same endpoint the desktop client uses; deployment
  switches this literal to the DDNS hostname once configured)
- Pinned fingerprint:
  `b9846aed2e97bd741ae5a2a3de9ab37c1831d2372ca67f26f538bd279dd7271f`
  (public pinning material, not a secret)

No external VPN or network client is required. Harbor tries direct IPv6,
direct IPv4/hole punching, then its own relay fallback. Settings carries
no server section by design — the desktop runbook records the same
product rule: the UI never shows addresses, fingerprints, or TLS details.
...
- Peer-side fan-out (direct-channel `device_hello`, multi-endpoint
  ...
  implemented in the core and covered by host tests. Historical live proof
  (2026-09-05, emulator → K11+): real pairing submit on the
...
## Oracle-default builds

Fresh installs point at K11 unless the build bakes an Oracle default. Bake it
at configure time from the operator environment (never commit the value —
`build-*/` is gitignored, but keep it out of shared logs too):

```sh
HARBOR_ORACLE_ADDRESS=<oracle-ipv4>:9091 ./build-android.sh arm64
```

Empty (default, and what CI uses) keeps the K11 literal fallback. The same
variable drives desktop builds (`cmake -S . -B build`) and the
`HARBOR_SERVER_DEFAULT_ADDRESS` runtime override still wins over the baked
value on desktop.
