...
ApplicationWindow {
    ...
    property bool serverNeedsMigration: false
    ...
    signal openPairing()
    signal closePairing()
    signal invitePairing(string harborId)
    signal copyHarborId()
    ...
    MobilePairingSheet {
        ...
        z: 11
        theme: appTheme
        ownHarborId: shell.ownHarborId
        incomingHarborId: shell.incomingHarborId
        phase: shell.pairingPhase
        errorText: shell.pairingError
        serverConfigured: shell.serverConfigured
        busy: shell.pairingBusy
            onConnectWithId: harborId => shell.invitePairing(harborId)
            onCopyId: shell.copyHarborId()
