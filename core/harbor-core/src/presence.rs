...
fn desired_state(snapshot: &UserActivitySnapshot) -> Option<PresenceState> {
    ...
    let fresh_input =
        matches!(snapshot.input_idle_seconds, Some(idle) if idle < AWAY_CANDIDATE_IDLE_SECS);
...
fn confirmation_for(state: PresenceState, snapshot: &UserActivitySnapshot) -> u64 {
    match state {
        ...
        PresenceState::Away | PresenceState::Offline => {
            let authoritative =
                snapshot.screen_locked == Some(true) || snapshot.session_active == Some(false);
            if authoritative {
                LOCK_CONFIRM_SECS
            } else {
                AWAY_CONFIRM_SECS
...
impl PartnerPresence {
    ...
    fn commit(&mut self, state: PresenceState, _now: u64) -> bool {
        self.previous = Some(
            self.state
                .replace(state)
                .expect("commit only follows a baseline"),
        );
        self.offline_since = None;
        self.revision = self.revision.saturating_add(1);
        true
...
mod tests {
    ...
    fn five_idle_minutes_are_a_candidate_that_confirms_late() {
        ...
        assert!(machine.observe(
            &snapshot(
                Some(AWAY_CANDIDATE_IDLE_SECS + AWAY_CONFIRM_SECS),
                None,
                None
            ),
            now
    ...
    fn a_candidate_dies_when_fresh_input_returns_before_committing() {
        ...
        assert!(!machine.observe(
            &snapshot(Some(AWAY_CANDIDATE_IDLE_SECS + 5), None, None),
            now
        ));
    ...
    fn returning_evidence_commits_online_quickly() {
        ...
        now += AWAY_CONFIRM_SECS;
        assert!(machine.observe(
            &snapshot(Some(away_age + AWAY_CONFIRM_SECS), None, None),
            now
        ));
        assert_eq!(machine.state(), PresenceState::Away);
    ...
    fn a_film_keeps_the_viewer_online_through_a_long_idle() {
        ...
        now += forty_minutes;
        assert!(!machine.observe(
            &snapshot(Some(forty_minutes), None, Some(MediaSignal::Playing)),
            now
        ));
        assert_eq!(machine.state(), PresenceState::Online);
    }
    ...
    fn a_paused_film_is_worth_nothing_and_idle_takes_over() {
        ...
        assert!(!machine.observe(
            &snapshot(
                Some(AWAY_CANDIDATE_IDLE_SECS + 10),
                None,
                Some(MediaSignal::Paused)
            ),
            now
        ));
        now += AWAY_CONFIRM_SECS;
        assert!(machine.observe(
            &snapshot(
                Some(AWAY_CANDIDATE_IDLE_SECS + 10 + AWAY_CONFIRM_SECS),
                None,
                Some(MediaSignal::Paused)
            ),
            now
    ...
    fn a_stopped_film_lets_the_idle_candidate_proceed() {
        ...
        assert!(!machine.observe(
            &snapshot(
                Some(AWAY_CANDIDATE_IDLE_SECS),
                None,
                Some(MediaSignal::Stopped)
            ),
            now
        ));
        now += AWAY_CONFIRM_SECS;
    ...
    fn a_lock_commits_away_quickly_and_outvotes_media() {
        ...
        assert!(!machine.observe(
            &snapshot(Some(5), Some(true), Some(MediaSignal::Playing)),
            now
        ));
        now += LOCK_CONFIRM_SECS;
        assert!(machine.observe(
            &snapshot(
                Some(5 + LOCK_CONFIRM_SECS),
                Some(true),
                Some(MediaSignal::Playing)
            ),
            now
        ));
        assert_eq!(machine.state(), PresenceState::Away);
    ...
    fn an_unlock_with_input_restores_online() {
        ...
        now += LOCK_CONFIRM_SECS;
        assert!(machine.observe(
            &snapshot(Some(60 + LOCK_CONFIRM_SECS), Some(true), None),
            now
        ));
        assert_eq!(machine.state(), PresenceState::Away);
    ...
    fn transitions_never_flap_inside_the_minimum_dwell() {
        ...
        assert!(!machine.observe(
            &snapshot(Some(2 + LOCK_CONFIRM_SECS), Some(true), None),
            now
        ));
        assert_eq!(machine.state(), PresenceState::Online);
        ...
        assert!(machine.observe(
            &snapshot(
                Some(2 + LOCK_CONFIRM_SECS + MIN_STATE_DWELL_SECS),
                Some(true),
                None
            ),
            now
    ...
    fn partner_offline_needs_a_streak_of_healthy_answers() {
        ...
        assert!(!partner.observe(
            PresenceState::Offline,
            1_040 + PARTNER_OFFLINE_CONFIRM_SECS - 1
        ));
