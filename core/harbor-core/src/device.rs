...
mod tests {
    ...
    fn battery_state_accepts_full_range_only() {
        ...
        assert_eq!(status.validate(), Err(MobileStatusError::BatteryOutOfRange));
    }
    ...
    fn phone_activity_on_carries_app_and_last_seen() {
        let status =
            MobileStatus::from_platform(&full_signals(), &sharing_intents(), 1_700_000_200);
    ...
    fn invented_app_labels_are_refused() {
        ...
        assert_eq!(
            status.validate(),
            Err(MobileStatusError::AppWithoutActivity)
        );
