...
pub fn validate_hello(raw: &str) -> Option<ValidHello> {
    ...
    if avatar.is_empty() {
        ...
        } else if avatar_hash.len() != 64 || !avatar_hash.bytes().all(|b| b.is_ascii_hexdigit()) {
            return None;
        }
    ...
    pub fn store(&mut self, chunk: &ValidChunk) -> Option<String> {
        ...
        for slot in &self.chunks {
            raw.extend_from_slice(
                &STANDARD
                    .decode(slot.as_ref().expect("complete"))
                    .unwrap_or_default(),
            );
        }
        ...
        match kind.as_deref() {
            ...
            Some("avatar_cancel") => {
                ...
                if self
                    .outbound
                    .as_ref()
                    .is_some_and(|o| o.hash == cancel.hash)
                {
                    self.outbound = None;
                }
            }
...
mod tests {
    ...
    fn malformed_hellos_are_rejected() {
        ...
        let long = format!(
            "{{\"t\":\"hello\",\"v\":1,\"revision\":1,\"display_name\":\"{}\",\"avatar_type\":\"none\",\"avatar_hash\":\"\",\"avatar\":\"\"}}",
            "x".repeat(81)
        );
    ...
    fn bogus_offers_and_chunks_are_rejected() {
        assert!(
            validate_offer(
                r#"{"t":"avatar_offer","v":1,"revision":1,"hash":"zz","size":10,"chunks":1}"#
            )
            .is_none()
        );
        ...
        assert!(
            validate_ref(
                r#"{"t":"avatar_request","v":1,"revision":1,"hash":"short"}"#,
                "avatar_request"
            )
            .is_none()
        );
    ...
    fn partner_snapshot_persists_and_reloads() {
        let directory =
            std::env::temp_dir().join(format!("harbor-profile-cache-{}", uuid::Uuid::new_v4()));
    ...
    fn corrupt_snapshot_loads_empty() {
        ...
        assert_eq!(
            PartnerSnapshot::load(&directory),
            PartnerSnapshot::default()
        );
        let _ = std::fs::remove_dir_all(&directory);
    }
}
