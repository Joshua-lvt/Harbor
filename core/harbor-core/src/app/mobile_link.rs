...
/// File transfer frames: offers, acceptances, chunks, cancels, and receipts.
/// Same units as the worker DataChannel path (`transfer_id`, chunk
/// `(seq, offset, bytes, final)`), so either transport feeds one board.
/// Bearer legs (mobile) carry these over direct TCP or sealed relay frames;
/// desktop legs use the worker until the Phase 3 bridge lands.
pub const KIND_FILE_OFFER: &str = "file_offer";
pub const KIND_FILE_ACCEPT: &str = "file_accept";
pub const KIND_FILE_REJECT: &str = "file_reject";
pub const KIND_FILE_CHUNK: &str = "file_chunk";
pub const KIND_FILE_CANCEL: &str = "file_cancel";
pub const KIND_FILE_RECEIVED: &str = "file_received";
...
/// The addresses one invite carries, in dial order: reflexive
/// (server-observed, Internet-meaningful) first when known, then the local
/// source addresses, loopback last (which an adb-reverse tunnel turns into
/// the real path on emulators). Capped at the invite limit; the dialer tries
/// each with a bounded timeout, so order is preference, never correctness.
pub fn invite_addrs(reflexive: Option<SocketAddr>, locals: Vec<String>) -> Vec<String> {
    let mut addrs = Vec::new();
    if let Some(addr) = reflexive {
        if let Some(text) = candidate_string(addr.ip()) {
            addrs.push(text);
        }
    }
    for local in locals {
...
/// The addresses this machine most likely answers on: the source IPs toward
/// the control-plane server over IPv4 and IPv6, plus loopback. Both families
/// are probed because a v6-only route to the server (or a v4-only host) must
/// not come back empty-handed; a UDP `connect` sends no packets, it only
/// reveals the local source the OS would use.
pub fn local_dial_addrs(server_addr: &str) -> Vec<String> {
    ...
    for bind in ["0.0.0.0:0", "[::]:0"] {
        ...
        if let Ok(local) = socket.local_addr() {
            if let Some(text) = candidate_string(local.ip()) {
...
/// A dialable invite literal: loopback and unspecified never route to a peer,
/// and IPv4-mapped IPv6 normalizes to plain IPv4 so one interface is never
/// advertised twice. Normalization runs before filtering because mapped
/// loopback (`::ffff:127.0.0.1`) is not loopback until unmapped.
fn candidate_string(ip: IpAddr) -> Option<String> {
    ...
    if normalized.is_loopback() || normalized.is_unspecified() {
        return None;
    }
    Some(normalized.to_string())
}

/// Reflexive UDP endpoint as the control-plane server sees it: the address a
/// direct dial should advertise to Internet peers. `server_udp` is the server
/// host with the same port over UDP (STUN-lite shares the listener's
/// address). One fresh nonce per query; echoes for another nonce are ignored,
/// never trusted. Bounded by `timeout`, never panics.
pub fn stun_reflexive(server_udp: &str, timeout: Duration) -> Option<SocketAddr> {
    let socket = UdpSocket::bind("[::]:0")
        .or_else(|_| UdpSocket::bind("0.0.0.0:0"))
        .ok()?;
    let nonce = visit_nonce();
    let request = stun::request_bytes(&nonce)?;
    socket.send_to(&request, server_udp).ok()?;
    ...
    loop {
        ...
        let (count, _) = socket.recv_from(&mut buffer).ok()?;
        if let Some(echo) = stun::parse_response(&buffer[..count], &nonce) {
            return Some(SocketAddr::new(echo.address, echo.port));
        }
    }
}

/// Fresh per-query correlation: process id, nanos, and a counter, hexed.
/// Uniqueness per query is what matters (a replayed echo answers nothing
/// new); secrecy is not required — the echo carries no authority by itself.
fn visit_nonce() -> String {
...
fn generate_identity() -> Option<(Vec<u8>, Vec<u8>)> {
    ...
    Some((certificate.der().to_vec(), key_pair.serialize_der()))
}
...
fn verify_signature(public_key: &str, message: &[u8], signature_hex: &str) -> bool {
    ...
    let (Some(key_bytes), Ok(signature)) = (key_bytes, Signature::from_slice(&signature_bytes))
    else {
        return false;
    };
...
fn sign_nonce(context: &LinkContext, nonce: &[u8]) -> [u8; 64] {
    ...
    SigningKey::from_bytes(&context.signing_seed)
        .sign(nonce)
        .to_bytes()
}
...
fn accept_one(
    ...
    let challenge =
        LinkFrame::new(KIND_CHALLENGE, json!({"nonce": hex(&nonce)})).expect("constant challenge");
...
fn dial_one(
    ...
    // Happy Eyeballs: IPv6 first (stable sort keeps invite preference inside
    // each family), fastest established socket wins. The loop below runs zero
    // or one times; `continue` still means "this path failed", exactly like
    // the old per-address ladder it replaces.
    let mut targets: Vec<SocketAddr> = invite
        .addrs
        .iter()
        .map(|addr| socket_addrs(addr, invite.port))
        .collect();
    targets.sort_by_key(|addr| !addr.is_ipv6());
    for socket in direct_conn::race_connect(
        &targets,
        LINK_DIAL_TIMEOUT,
        direct_conn::RACE_OVERALL_TIMEOUT,
    ) {
        if shutdown_requested(shutdown) {
            return;
        }
        ...
        let client_conn = match rustls::ClientConnection::new(
            client_config(invite.fingerprint),
            link_server_name(),
        ) {
            Ok(conn) => conn,
            Err(_) => continue,
        ...
        let peer = loop {
            ...
            if frame.kind == KIND_CHALLENGE {
                ...
                let answer =
                    LinkFrame::new(KIND_CHALLENGE_RESPONSE, json!({"signature": signature}))
...
mod tests {
    ...
    fn invites_parse_tightly_or_not_at_all() {
        ...
        assert!(
            LinkInvite::parse(&json!({"addrs": [], "port": 1, "fingerprint": "ab".repeat(32)}))
                .is_none()
        );
        assert!(
            LinkInvite::parse(&json!({"addrs": ["x"], "port": 0, "fingerprint": "ab".repeat(32)}))
                .is_none()
        );
        assert!(
            LinkInvite::parse(&json!({"addrs": ["x"], "port": 1, "fingerprint": "zz"})).is_none()
        );
        let mut many = vec![];
        for _ in 0..9 {
        ...
        assert!(
            LinkInvite::parse(&json!({"addrs": many, "port": 1, "fingerprint": "ab".repeat(32)}))
                .is_none()
        );
    }
    ...
    #[test]
    fn local_addrs_skip_loopback_on_both_families() {
        // UDP connect sends nothing; loopback sources filter out on either
        // family and the adb-reverse fallback stays last.
        assert_eq!(local_dial_addrs("127.0.0.1:9"), vec!["127.0.0.1"]);
        assert_eq!(local_dial_addrs("[::1]:9"), vec!["127.0.0.1"]);
        assert_eq!(local_dial_addrs("not a socket at all"), vec!["127.0.0.1"]);
    }

    #[test]
    fn candidate_strings_normalize_and_filter() {
        assert_eq!(candidate_string("127.0.0.1".parse().unwrap()), None);
        assert_eq!(candidate_string("::1".parse().unwrap()), None);
        assert_eq!(candidate_string("0.0.0.0".parse().unwrap()), None);
        assert_eq!(candidate_string("::".parse().unwrap()), None);
        assert_eq!(
            candidate_string("192.168.1.6".parse().unwrap()).as_deref(),
            Some("192.168.1.6")
        );
        assert_eq!(
            candidate_string("2001:db8::1".parse().unwrap()).as_deref(),
            Some("2001:db8::1")
        );
        // One interface is never advertised twice via mapping.
        assert_eq!(
            candidate_string("::ffff:192.168.1.6".parse().unwrap()).as_deref(),
            Some("192.168.1.6")
        );
    }

    #[test]
    fn invite_addrs_order_reflexive_first_dedupe_and_cap() {
        let reflexive: SocketAddr = "[2001:db8::1]:41234".parse().unwrap();
        let locals = vec!["192.168.1.6".to_owned(), "127.0.0.1".to_owned()];
        assert_eq!(
            invite_addrs(Some(reflexive), locals),
            vec!["2001:db8::1", "192.168.1.6", "127.0.0.1"]
        );

        // A reflexive that matches a local source appears once.
        let same: SocketAddr = "192.168.1.6:41234".parse().unwrap();
        assert_eq!(
            invite_addrs(Some(same), vec!["192.168.1.6".to_owned()]),
            vec!["192.168.1.6"]
        );

        // No reflexive: locals pass through, still capped at eight.
        let many: Vec<String> = (1..=10).map(|n| format!("10.0.0.{n}")).collect();
        let capped = invite_addrs(None, many);
        assert_eq!(capped.len(), 8);
        assert_eq!(capped[0], "10.0.0.1");
    }

    #[test]
    fn stun_reflexive_echoes_through_a_real_server() {
        let server = harbor_server::StunServer::spawn(
            "127.0.0.1:0".parse().unwrap(),
            std::sync::Arc::new(std::sync::Mutex::new(harbor_server::TurnState::new())),
        )
        .unwrap();
        let target = server.local_addr().to_string();
        let echo = stun_reflexive(&target, Duration::from_secs(2)).expect("loopback echo answers");
        assert_eq!(echo.ip().to_string(), "127.0.0.1");
        assert_ne!(echo.port(), 0);
        server.shutdown();
    }

    #[test]
    fn stun_reflexive_gives_up_on_a_dead_port() {
        let probe = std::net::UdpSocket::bind("127.0.0.1:0").unwrap();
        let dead = probe.local_addr().unwrap().port();
        drop(probe);
        let target = format!("127.0.0.1:{dead}");
        assert!(stun_reflexive(&target, Duration::from_millis(300)).is_none());
    }
}
