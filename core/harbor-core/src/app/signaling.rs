...
use crate::{LocalIdentity, ServerClient, ServerPin, load_or_create, load_server_pin, rfc3339_now};
...
impl Signaling {
    ...
    /// Raw request/response keeping the refusal envelope: relay control
    /// needs the refusal *code* (reopen vs back off vs drop), not just a
    /// boolean. Same connection discipline as [`Signaling::exchanged`].
    fn exchanged_value(
        &mut self,
        identity: &LocalIdentity,
        message_type: &str,
        ...
        match client.exchange(request, identity) {
            Ok(response) => match response.error {
                ...
                Some(error) => Err(ExchangeFailure::Refused(error)),
            },
            Err(_) => {
                self.client = None;
                Err(ExchangeFailure::Transport)
            }
        }
    }
    ...
    fn tick_inner(&mut self, state: &mut CoreState) -> SignalingTick {
        ...
        for peer in &peers {
            let session = match self.exchanged(
                &identity,
                "session.connect",
                json!({"peer": peer.device_id}),
            ) {
                Ok(payload) => payload,
                Err(_) => {
                    self.fail();
    ...
    fn inbound(
        ...
        match signal.get("type").and_then(Value::as_str)? {
            ...
            // Relay key halves for a session handshake. The session owner is
            // server-authenticated (signals route only within the pair
            // session); shape-checked here, matched to a session on the pump.
            "relay_keys" => {
                self.peer_harbor(&from_peer)?;
                let relay_id = signal
                    .get("relay_id")
                    .and_then(Value::as_str)
                    .and_then(|id| Uuid::parse_str(id).ok());
                let public = signal
                    .get("eph_pub")
                    .and_then(Value::as_str)
                    .and_then(super::relay_client::parse_eph_pub);
                if let (Some(relay_id), Some(public)) = (relay_id, public) {
                    state.pending_relay_keys.push((relay_id, from_peer, public));
                }
                None
            }
            "offer" => {
    ...
    fn prepare_inner(&mut self, state: &mut CoreState) -> Result<Uuid, ProtocolError> {
        ...
        self.resolve_peers(&identity, true)
...
/// A relay control exchange that distinguishes a server refusal (with its
/// code) from a transport failure. `Refused` carries the refusal envelope's
/// structured error.
#[derive(Debug)]
...
/// Fetches short-lived TURN credentials for the media worker. TURN is a
/// fallback, so a temporary credential failure leaves direct ICE available.
pub fn turn_credentials(state: &mut CoreState) -> Option<Value> {
    let pin = load_server_pin(&state.state_dir)?;
    let mut signaling = std::mem::take(&mut state.signaling);
    let result = (|| {
        let identity = signaling.load_identity(&state.state_dir)?;
        signaling.open(&pin, &state.state_dir).ok()?;
        let payload = signaling
            .exchanged(&identity, "turn.credentials", json!({}))
            .ok()?;
        let username = payload.get("username")?.as_str()?;
        let password = payload.get("password")?.as_str()?;
        let realm = payload.get("realm")?.as_str()?;
        Some(json!({
            "username": username,
            "credential": password,
            "realm": realm,
            "ttl_secs": payload.get("ttl_secs").and_then(Value::as_u64),
        }))
    })();
    state.signaling = signaling;
    result
...
/// Relay control exchange over the reused control connection: open, accept,
/// close, and data (the parked poll has its own worker loop). Unlike
/// [`send_to`], refusals keep their code so the caller can reopen, back off,
/// or drop the session accordingly.
pub fn relay_exchange(
    ...
    if signaling.open(&pin, &state.state_dir).is_err() {
        state.signaling = signaling;
        return Err(ExchangeFailure::Transport);
    }
    let result = signaling.exchanged_value(&identity, message_type, payload);
    state.signaling = signaling;
    result
}
...
mod tests {
    ...
    fn a_validated_hello_learns_the_peer_kind() {
        ...
        let tick = signaling.inbound(
            &mut core_state,
            &hello_from(peer, "mobile", "harbor-abc"),
            peer,
        );
        assert!(tick.is_none());
    ...
    fn a_mobile_core_dials_a_validated_invite_and_desktops_ignore_it() {
        ...
        assert!(tick.is_none());
        assert!(core_state.link_invite.is_some(), "a validated invite dials");
