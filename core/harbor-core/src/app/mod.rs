...
use self::mobile_link::{
    KIND_CHAT, KIND_CHAT_ACK, KIND_FILE_ACCEPT, KIND_FILE_CANCEL, KIND_FILE_CHUNK, KIND_FILE_OFFER,
    KIND_FILE_RECEIVED, KIND_FILE_REJECT, KIND_HELLO, KIND_MOBILE, KIND_PHONE_NOTIFICATION,
    KIND_PROFILE, LinkContext, LinkFrame, LinkInvite, LinkPeer, MobileLink, hello_frame,
    invite_addrs, local_dial_addrs, stun_reflexive,
};
use self::relay_client::{
    RelayPollWorker, RelaySession, RelaySessions, SessionCrypto, eph_pub_hex, open_frame,
    relay_wanted, seal_frame,
};
use self::relay_crypto::PendingCrypto;

use self::signaling::{SignalingTick, server_unavailable_error};

use crate::activity::ActivityEngine;
use crate::device::{
    DeviceRecord, DeviceType, MobileStatus, initial_mic_muted, own_mode, reconnect_delay,
};
use crate::direct::{ChatSession, Delivery, Direction, TransferBoard, TransferPhase};
use crate::{
    PairingError, PairingSession, PinMigration, ServerPin, Settings, SettingsError,
    is_migration_source, load_or_create, load_server_pin, migrate_server_pin, oracle_endpoint,
    store_server_pin,
};
...
struct CoreState {
    ...
    /// Final relay file receipts waiting for a successful server enqueue.
    /// Transfer completion is idempotent, so retries are safe across ticks.
    pending_relay_receipts: Vec<(Uuid, String, bool)>,
    direct_dirty: bool,
    ...
    /// Relay fallback sessions keyed by peer device, plus the poll worker
    /// and inbound key halves awaiting a session match. Transient: server
    /// expiry plus reopen-on-demand replace persistence.
    relay: RelaySessions,
    pending_relay_keys: Vec<(Uuid, Uuid, [u8; 32])>,
    relay_worker: RelayPollWorker,
...
impl CoreState {
    ...
    fn from_default() -> Self {
        ...
        Self {
            ...
            pending_relay_receipts: Vec::new(),
            direct_dirty: false,
            ...
            relay: RelaySessions::default(),
            pending_relay_keys: Vec::new(),
            relay_worker: RelayPollWorker::new(),
            mic_test: None,
        }
    }
    ...
    fn device_snapshot(&self) -> Result<Value, ProtocolError> {
        ...
        let device_type = DeviceType::parse(settings.values().device_type.as_str())
    ...
    fn send_link_invite(&mut self, peer: &Uuid) {
        ...
        // Reflexive (server-observed, Internet-meaningful) first, then local
        // source addresses, loopback last. One bounded STUN round trip on the
        // same host/port over UDP; invites only go out on listener/leg
        // changes, never per tick, so the round trip never wedges the pump.
        let addrs = invite_addrs(
            stun_reflexive(&server_addr, Duration::from_secs(2)),
            local_dial_addrs(&server_addr),
        );
        signaling::send_to(
            self,
            peer,
    ...
    fn send_link_profile_frame(&mut self, frame: &str) -> bool {
        ...
        self.relay_send_frame(&frame.kind, frame.payload)
    }
    ...
    /// Pushes our validated phone aggregate to the peer. Called on every
    /// `mobile.update` and on link rise; the peer renders it through
    /// `mobile.state`, never stores it beyond the session view. Rides the
    /// bearer when live, the relay otherwise.
    fn transmit_link_mobile(&mut self) {
        ...
        if let Some(frame) = LinkFrame::new(KIND_MOBILE, status) {
            self.transmit_bearer_frame(frame);
        }
    }

    /// Sends one display-only phone notification over the authenticated
    /// mobile path (bearer, else relay). The queue is intentionally not
    /// retained: an offline peer misses the notification rather than
    /// receiving it later from durable storage.
    ...
    fn absorb_link_frame(&mut self, peer: Uuid, frame: LinkFrame) {
        match frame.kind.as_str() {
                ...
                if let (Some(id), Some(body)) = (id, body) {
                        ...
                        .record_inbound(id.to_owned(), body.to_owned(), now_seconds())
                    {
                        ...
                        if let Some(ack) = LinkFrame::new(KIND_CHAT_ACK, json!({"id": id})) {
                            // The ack returns over whatever carried the chat:
                            // direct bearer when live, relay otherwise.
                            self.transmit_bearer_frame(ack);
                        }
                    }
                }
            }
            ...
            KIND_FILE_OFFER => {
                let id = frame.payload.get("transfer_id").and_then(Value::as_str);
                let name = frame.payload.get("name").and_then(Value::as_str);
                let size = frame.payload.get("size").and_then(Value::as_u64);
                let sum = frame.payload.get("sha256").and_then(Value::as_str);
                if let (Some(id), Some(name), Some(size), Some(sum)) = (id, name, size, sum) {
                    // Duplicate offers (re-announce after session rotation,
                    // replayed frames) converge: one entry per transfer id.
                    // A duplicate for a live incoming transfer re-sends the
                    // accept, healing rotations without waiting for expiry.
                    ...
                    {
                        if existing.direction == Direction::Incoming
                            && existing.phase == TransferPhase::Active
                        {
                            self.relay_send_frame(KIND_FILE_ACCEPT, json!({"transfer_id": id}));
                        }
                        return;
                    }
                    ...
                    if self
                        .transfers
                        .offer_inbound(id.into(), name, size, sum.into(), now_seconds())
                        .is_ok()
                    {
                        self.direct_dirty = true;
                    }
            ...
            KIND_FILE_ACCEPT => {
                if let Some(id) = frame.payload.get("transfer_id").and_then(Value::as_str) {
                    if self.transfers.observe_accepted(id) {
                        self.direct_dirty = true;
                    }
                }
            }
            KIND_FILE_REJECT | KIND_FILE_CANCEL => {
                if let Some(id) = frame.payload.get("transfer_id").and_then(Value::as_str) {
                    if self.transfers.observe_canceled(id) {
                        self.direct_dirty = true;
                    }
                }
            }
            KIND_FILE_CHUNK => {
                let id = frame.payload.get("transfer_id").and_then(Value::as_str);
                let seq = frame.payload.get("seq").and_then(Value::as_u64);
                let offset = frame.payload.get("offset").and_then(Value::as_u64);
                let data = frame
                    .payload
                    .get("data")
                    .and_then(Value::as_str)
                    .and_then(|text| base64::engine::general_purpose::STANDARD.decode(text).ok());
                let final_chunk = frame
                    .payload
                    .get("final")
                    .and_then(Value::as_bool)
                    .unwrap_or(false);
                let (Some(id), Some(seq), Some(offset), Some(data)) = (id, seq, offset, data)
                else {
                    return;
                };
                ...
                if self
                    .transfers
                    .write_inbound_chunk(id, seq, offset, &data, final_chunk)
                    .is_err()
                {
                    return;
                }
                ...
                if final_chunk {
                    ...
                    let ok = self
                        .transfers
                        .finalize_inbound(id, &staging, &destination)
                        .is_ok();
                    self.queue_relay_receipt(peer, id, ok);
                }
            }
            KIND_FILE_RECEIVED => {
                let id = frame.payload.get("transfer_id").and_then(Value::as_str);
                let ok = frame
                    ...
                    .unwrap_or(false);
                if let Some(id) = id {
                    // A failed peer verification surfaces honestly instead of
                    // stranding a fully-sent transfer as unconfirmed.
                    let changed = if ok {
                        self.transfers.observe_peer_received(id)
                    } else {
                        self.transfers.observe_canceled(id)
                    };
                    if changed {
                        self.direct_dirty = true;
                    }
    ...
    /// Relay fallback pump: ingest poll deliveries, match key halves,
    /// advance handshakes, open sessions for queued chat without a direct
    /// path. All exchanges are bounded; failures cool down per peer.
    fn pump_relay(&mut self) {
        for event in self.relay_worker.drain() {
            match event {
                relay_client::RelayEvent::Polled {
                    frames,
                    closed,
                    opens,
                } => {
                    for id in closed {
                        self.relay.remove_by_id(&id);
                    }
                    for notice in opens {
                        self.note_relay_open(notice.relay_id, notice.from);
                    }
                    for frame in frames {
                        self.ingest_relay_frame(frame);
                    }
                }
            }
        }
        self.consume_relay_keys();
        self.maybe_open_relay();
        self.advance_relay_handshakes();
        if self.relay.is_empty() {
            self.relay_worker.idle_stop();
        } else {
            if let Some(pin) = load_server_pin(&self.state_dir) {
                self.relay_worker.ensure_worker(&pin, &self.state_dir);
            }
            // Profile rides ready sessions even with no bearer: the flush is
            // idempotent (hello per revision, chunks advance on success), so
            // sharing it with the bearer-live branch below is safe.
            if !self.relay.ready_peers().is_empty() {
                self.flush_link_profile();
            }
            self.pump_relay_files();
        }
    }

    /// Whether the worker DataChannel path currently owns file traffic: an
    /// established call with a live media worker. Relay file frames flow
    /// only outside it, so the two transports never double-send a chunk
    /// (and the board's strict ordering would reject the duplicate anyway).
    fn worker_covers_files(&self) -> bool {
        self.call.phase == "CONNECTED" && self.call.call_id.is_some() && self.media.is_some()
    }

    /// Relay file pump, one chunk per active outgoing transfer per tick
    /// (same pacing rationale as the worker path), plus offer announce.
    /// Runs only while relay sessions are ready and the worker path does not
    /// cover files. Throttle-gated against the shared uplink budget; paused
    /// chunks stay `pending` in the board and resume on later ticks — across
    /// relay sessions too, since file offsets are transport-free.
    fn pump_relay_files(&mut self) {
        self.flush_relay_receipts();
        if self.worker_covers_files() {
            return;
        }
        for record in self.transfers.records() {
            ...
            if record.phase == TransferPhase::Offered {
                if let Some((name, size, sum)) = self.transfers.outgoing_offer(&record.id) {
                    self.announce_relay_offer(&record.id, name, size, sum);
                }
                continue;
            }
            if record.phase != TransferPhase::Active {
                continue;
            }
            if !self
                .relay
                .throttle_try_consume(crate::direct::TRANSFER_CHUNK_BYTES)
            {
                break;
            }
            let Ok(Some((seq, offset, data, done))) =
            ...
            if self.relay_send_frame(KIND_FILE_CHUNK, payload) {
    ...
    fn queue_relay_receipt(&mut self, peer: Uuid, transfer_id: &str, ok: bool) {
        if let Some(receipt) = self
            .pending_relay_receipts
            .iter_mut()
            .find(|(target, id, _)| *target == peer && id == transfer_id)
        {
            receipt.2 = ok;
        } else {
            self.pending_relay_receipts
                .push((peer, transfer_id.to_owned(), ok));
        }
        self.flush_relay_receipts();
    }

    fn flush_relay_receipts(&mut self) {
        for (peer, transfer_id, ok) in self.pending_relay_receipts.clone() {
            if self.relay_send_frame_to(
                peer,
                KIND_FILE_RECEIVED,
                json!({"transfer_id": transfer_id, "ok": ok}),
            ) {
                self.pending_relay_receipts
                    .retain(|(target, id, _)| *target != peer || id != &transfer_id);
            }
        }
    }

    /// A poll-delivered open from a paired peer: stage a session so the
    /// handshake advance accepts it and completes on keys arrival. Strangers
    /// never reach here authenticated, but the worker parses untrusted
    /// bytes — recheck pairing cheaply.
    fn note_relay_open(&mut self, relay_id: Uuid, from: Uuid) {
        if self.signaling.peer_harbor(&from).is_none() {
            return;
        }
        if self.relay.session_for_id(&relay_id).is_some()
            || self.relay.session_for_peer(&from).is_some()
        {
            return;
        }
        let pending = PendingCrypto::generate();
        let eph_pub = pending.public_bytes();
        self.relay.insert_if_absent(RelaySession {
            relay_id,
            peer: from,
    ...
    /// Matches inbound key halves (via `relay_keys` signals) to sessions:
    /// completes crypto for the opener side, stages acceptor sessions for
    /// unknown ids from paired peers. Freshly completed sessions drain
    /// their inbox: frames the peer sealed before we completed open in
    /// arrival order instead of stranding on sequence zero forever.
    fn consume_relay_keys(&mut self) {
        let pending = std::mem::take(&mut self.pending_relay_keys);
        for (relay_id, from, public) in pending {
            if !self.relay.complete_with(&relay_id, &from, public) {
                if self.signaling.peer_harbor(&from).is_none() {
                    continue;
                }
                if self.relay.session_for_id(&relay_id).is_some()
                ...
                let pending = PendingCrypto::generate();
                let eph_pub = pending.public_bytes();
                if !self.relay.insert_if_absent(RelaySession {
                    relay_id,
                    peer: from,
                    crypto: SessionCrypto::Pending(pending),
                    eph_pub,
                ...
                self.relay.complete_with(&relay_id, &from, public);
            }
            self.absorb_inbox(&relay_id);
        }
    }

    /// Opens queued pre-completion frames through fresh crypto, in arrival
    /// order, through the exact bearer path. Strict sequence still enforced
    /// per frame; undecryptable entries drop like any other.
    fn absorb_inbox(&mut self, relay_id: &Uuid) {
        for (seq, bytes) in self.relay.drain_inbox(relay_id) {
            let parsed = {
                let Some(session) = self.relay.session_for_id_mut(relay_id) else {
                    break;
                };
                open_frame(session, seq, &bytes)
                    .map(|(kind, payload)| (session.peer, kind, payload))
            };
            let Some((peer, kind, payload)) = parsed else {
                continue;
            };
            self.absorb_opened(peer, kind, payload);
        }
    }

    /// Applies one opened relay frame through the exact bearer path (same
    /// sanitizers, same transcript, same acks).
    fn absorb_opened(&mut self, peer: Uuid, kind: String, payload: Value) {
        let Some(frame) = LinkFrame::new(&kind, payload) else {
            return;
        };
        self.absorb_link_frame(peer, frame);
    }

    /// Accepts staged sessions and (re)sends our key half until the peer
    /// acknowledges keys. Acceptance is automatic for paired peers with
    /// server-side idle expiry; the explicit `relay.accept` message (with
    /// its 10 s window) still flows, so the server never relays on a bare
    /// open.
    fn advance_relay_handshakes(&mut self) {
        for (peer, relay_id) in self.relay.sessions_snapshot() {
            let (opened_by_me, accepted_sent, keys_sent, eph_pub) =
                match self.relay.session_for_id(&relay_id) {
                    Some(session) => (
                        session.opened_by_me,
                        session.accepted_sent,
                        session.keys_sent,
                        session.eph_pub,
                    ),
                    None => continue,
                };
            if !opened_by_me && !accepted_sent {
                match signaling::relay_exchange(self, "relay.accept", json!({"relay_id": relay_id}))
                {
                    Ok(_) => {
                        if let Some(session) = self.relay.session_for_id_mut(&relay_id) {
                            session.accepted_sent = true;
                        }
                    }
                    Err(signaling::ExchangeFailure::Refused(_)) => {
                        self.relay.remove_by_id(&relay_id);
                    }
                    Err(signaling::ExchangeFailure::Transport) => {}
                }
            }
            if !keys_sent {
                let signal = json!({
                    ...
                    "signal": {
                        ...
                        "eph_pub": eph_pub_hex(&eph_pub),
                    },
                });
                if signaling::send_to(self, &peer, &signal) {
                    if let Some(session) = self.relay.session_for_id_mut(&relay_id) {
                        session.keys_sent = true;
                    }
                }
            }
        }
    ...
    /// Opens a relay session when outbound traffic waits with no direct
    /// path: queued chat, a waiting outgoing transfer, or a final receipt.
    /// Normal traffic targets the mobile leg; a receipt may reopen toward
    /// its sole paired peer after the original session expires.
    /// Cooled down per peer so a failing server is probed politely.
    fn maybe_open_relay(&mut self) {
        let worker_covers =
            self.call.phase == "CONNECTED" && self.call.call_id.is_some() && self.media.is_some();
        let files_waiting = !worker_covers
            && self.transfers.records().iter().any(|record| {
                record.direction == Direction::Outgoing
                    && (record.phase == TransferPhase::Offered
                        || record.phase == TransferPhase::Active)
            });
        let receipts_waiting = !self.pending_relay_receipts.is_empty();
        if !files_waiting
            && !receipts_waiting
            && !relay_wanted(
                self.chat.queue_len(),
                self.link.live_peer().is_some(),
                worker_covers,
            )
        {
            return;
        ...
        let peer = match self
            .pending_relay_receipts
            .first()
            .and_then(|(peer, _, _)| self.signaling.legs().iter().find(|leg| leg.peer == *peer))
            .or_else(|| {
                self.signaling
                    .legs()
                    .iter()
                    .find(|leg| leg.device == Some(DeviceType::Mobile))
            })
            .map(|leg| leg.peer)
        {
            Some(peer) => peer,
        ...
        if self.relay.session_for_peer(&peer).is_some() {
            return;
        }
        if !self.relay.open_cooldown_ok(&peer, Instant::now()) {
            return;
        }
        match signaling::relay_exchange(
            self,
            "relay.open",
            json!({"peer": peer, "purpose": "chat"}),
        ) {
            Ok(payload) => {
                let Some(relay_id) = payload
                    .get("relay_id")
                    .and_then(Value::as_str)
                    .and_then(|id| Uuid::parse_str(id).ok())
                else {
                    self.relay.note_open_attempt(peer, Instant::now());
                    return;
                };
                let pending = PendingCrypto::generate();
                let eph_pub = pending.public_bytes();
    ...
    /// Applies one relay-delivered sealed frame: unseal, reshape into a
    /// bearer frame, and ingest through the exact bearer path (same
    /// sanitizers, same transcript, same acks — which route back over the
    /// relay when the bearer is down).
    fn ingest_relay_frame(&mut self, frame: relay_client::WireFrame) {
        let parsed = {
            let Some(session) = self.relay.session_for_id_mut(&frame.relay_id) else {
                return;
            };
            if session.peer != frame.from {
                return;
            }
            // Pre-completion arrivals park in the inbox (bounded, newest
            // drop); the completing pump drains them in order instead of
            // stranding on sequence zero forever.
            if !matches!(session.crypto, SessionCrypto::Ready(_)) {
                if session.inbox.len() < relay_client::INBOX_CAP {
                    session.inbox.push_back((frame.seq, frame.bytes));
                }
                return;
            }
            let parsed = open_frame(session, frame.seq, &frame.bytes);
            parsed.map(|(kind, payload)| (session.peer, kind, payload))
        };
        let Some((peer, kind, payload)) = parsed else {
            return;
        };
    ...
    /// Announces one outgoing offer over the first ready relay session,
    /// once per session. Repeats would crowd the bounded server queue and
    /// starve accepts and chunks; a fresh session (rotation, reopen)
    /// announces again, and receivers dedupe by id while re-accepting
    /// duplicates for live incoming transfers.
    fn announce_relay_offer(&mut self, transfer_id: &str, name: String, size: u64, sum: String) {
        let peer = match self.relay.ready_peers().first() {
            Some(peer) => *peer,
            None => return,
        };
        let (relay_id, announced) = match self.relay.session_for_peer(&peer) {
            Some(session) => (session.relay_id, session.offered.contains(transfer_id)),
            None => return,
        };
        if announced {
            return;
        }
        if self.relay_send_frame(
            KIND_FILE_OFFER,
            json!({"transfer_id": transfer_id, "name": name, "size": size, "sha256": sum}),
        ) {
            if let Some(session) = self.relay.session_for_id_mut(&relay_id) {
                session.offered.insert(transfer_id.to_owned());
            }
        }
    }

    /// Sends one bearer-protocol frame toward the first ready relay session
    /// (companion model: one conversation). Returns whether the frame left
    /// on the relay; failures drop the session when it is gone or cannot
    /// safely continue (reopen heals on demand).
    fn relay_send_frame(&mut self, kind: &str, payload: Value) -> bool {
        let peer = match self.relay.ready_peers().first() {
            Some(peer) => *peer,
            None => return false,
        };
        self.relay_send_frame_to(peer, kind, payload)
    }

    fn relay_send_frame_to(&mut self, peer: Uuid, kind: &str, payload: Value) -> bool {
        let Some(session) = self.relay.session_for_peer_mut(&peer) else {
            return false;
        };
        let (seq, bytes) = match relay_client::seal_frame(session, kind, &payload) {
            Some(sealed) => sealed,
            None => return false,
        };
        ...
        match signaling::relay_exchange(
            self,
            "relay.data",
            json!({"relay_id": relay_id, "seq": seq, "bytes": bytes}),
        ) {
            Ok(_) => true,
            // The sealed sequence cannot be reused even when queue refusal is
            // provably pre-enqueue. Retire the keys and let queued application
            // state reopen a fresh session rather than rolling the nonce back.
            Err(signaling::ExchangeFailure::Refused(error)) if error.code == "relay_busy" => {
                self.relay.remove_by_id(&relay_id);
                false
            }
            // The server reports the session gone (closed, unknown after a
            // restart, byte-exhausted) or desynced (order): drop it locally
            // and let the open trigger heal on demand. File offsets persist
            // in the board, so the next session resumes mid-transfer instead
            // of restarting.
            Err(signaling::ExchangeFailure::Refused(error))
                if error.code == "relay_closed"
                    || error.code == "relay_unknown"
                    || error.code == "relay_exhausted"
                    || error.code == "relay_order" =>
            {
                self.relay.remove_by_id(&relay_id);
                false
            }
    ...
    /// Sends one bearer-protocol frame, preferring the live direct bearer
    /// and falling back to a ready relay session. Returns whether the frame
    /// left this device on any transport.
    fn transmit_bearer_frame(&mut self, frame: LinkFrame) -> bool {
        if self.link.live_peer().is_some() {
            self.link.send(frame);
            return true;
        }
    ...
    fn flush_chat_queue(&mut self) {
        ...
        let worker_live =
            self.call.phase == "CONNECTED" && self.call.call_id.is_some() && self.media.is_some();
        // Relay counts as a live path: sealed frames leave even when no
        // direct transport exists. Without any path at all the bounded queue
        // simply waits.
        if !link_live && !worker_live && self.relay.ready_peers().is_empty() {
            return;
        }
        ...
        for message in queued {
            let mut sent = false;
            let frame = LinkFrame::new(KIND_CHAT, json!({"id": message.id, "body": message.body}));
            if link_live {
            ...
            if !sent {
                // Relay fallback for bearer legs: sealed frames over the
                // control connection; delivery marks stay transport-free and
                // acks flow back over the same path.
                if let Some(frame) = frame {
                    if self.relay_send_frame(&frame.kind, frame.payload) {
                        self.chat.mark_delivery(&message.id, Delivery::Sent);
                        self.direct_dirty = true;
                        sent = true;
                    }
            ...
            if worker_live {
                if let Some(media) = self.media.as_ref() {
                    match media.request(
                        ...
                        Ok(reply) if reply.error.is_none() && reply.message_type == "chat.send" => {
                            self.chat.mark_delivery(&message.id, Delivery::Sent);
                            self.direct_dirty = true;
                            sent = true;
                        }
    ...
    fn sync_direct(&mut self) {
        ...
        // bearer, relay, or several for a companion identity bridging peers.
        // Relay events land before the bearer pump so freshly completed
        // sessions deliver in the same tick.
        self.pump_relay();
        self.pump_link();
        let expired = self.transfers.expire(now_seconds());
        if !expired.is_empty() {
            self.direct_dirty = true;
            // Expired offers the peer may hold die quietly otherwise; tell
            // it over the relay when the worker path does not cover files.
            if !self.worker_covers_files() {
                for id in expired {
                    self.relay_send_frame(KIND_FILE_CANCEL, json!({"transfer_id": id}));
                }
            }
        }
    ...
    fn sync_profile_exchange(&mut self, call_id: &str, now: u64) -> bool {
        ...
        if self.profile.pending_hello(&local, now).is_some() {
            let frame = self.profile.pending_hello(&local, now).unwrap_or_default();
            if let Some(media) = self.media.as_ref() {
                match media.request("profile.send", json!({"call_id": call_id, "frame": frame})) {
                    Ok(reply) if reply.error.is_none() && reply.message_type == "profile.send" => {
                        self.profile.mark_hello_sent(local.revision, now);
                    }
                    _ => {}
                }
        ...
        for _ in 0..profile::PROFILE_AVATAR_CHUNKS_PER_TICK {
            ...
            let accepted = self
                ...
                .is_some_and(|reply| reply.error.is_none() && reply.message_type == "profile.send");
            if !accepted {
                break;
            }
            self.profile.advance_chunk();
...
fn dispatch(request: Envelope, state: &mut CoreState) -> Result<Envelope, CoreError> {
    ...
    match request.message_type.as_str() {
        ...
        "presence.sense" => match serde_json::from_value::<crate::presence::UserActivitySnapshot>(
        ...
        "server.config" => match load_server_pin(&state.state_dir) {
            Some(pin) => {
                response.payload = json!({
                    ...
                    "needs_migration": is_migration_source(&pin.address),
                    "migration_target": oracle_endpoint().unwrap_or_default(),
                })
            }
        ...
        // Idempotent K11 -> Oracle endpoint migration (E1-E5): only an exact
        // legacy endpoint migrates, the fingerprint is preserved from the
        // stored pin, and custom endpoints are never overwritten. The caller
        // must reconnect control/STUN/TURN/relay afterwards and offer a
        // controlled rollback (re-migrate is refused for non-legacy: rollback
        // is an explicit server.configure by the user, never silent).
        "server.migrate" => {
            // Pin first: fresh installs configure directly instead of
            // migrating, whatever the environment holds (this also keeps the
            // arm deterministic under parallel tests that set the vars).
            if load_server_pin(&state.state_dir).is_none() {
                response.error = Some(ProtocolError {
                    code: "server_unconfigured".into(),
                    ui_key: "error.server.unconfigured".into(),
                    retryable: false,
                    detail: "no server pin is configured; configure a fresh endpoint instead"
                        .into(),
                });
                state.drain_media_events();
                return Ok(response);
            }
            let to_address = request.payload.get("to_address").and_then(Value::as_str);
            // Back-compat: older UIs send "address".
            let to_address = to_address
                .or_else(|| request.payload.get("address").and_then(Value::as_str))
                .map(str::to_owned)
                .or_else(oracle_endpoint);
            let Some(to_address) = to_address else {
                response.error = Some(ProtocolError {
                    code: "invalid_request".into(),
                    ui_key: "error.server.invalidAddress".into(),
                    retryable: false,
                    detail:
                        "no migration destination: pass to_address or set HARBOR_ORACLE_ADDRESS"
                            .into(),
                });
                state.drain_media_events();
                return Ok(response);
            };
            match migrate_server_pin(&state.state_dir, &to_address) {
                Ok(PinMigration::Migrated { from, to }) => {
                    let pin = load_server_pin(&state.state_dir).expect("just migrated");
                    let needs = is_migration_source(&to);
                    response.payload = json!({
                        "migrated": true,
                        "from": from,
                        "address": to,
                ...
                Ok(PinMigration::AlreadyMigrated { address }) => {
                    let pin = load_server_pin(&state.state_dir).expect("pin exists");
                    response.payload = json!({
                        "migrated": false,
                        "address": address.clone(),
                        "fingerprint": pin.fingerprint_hex,
                        "configured": true,
                        "needs_migration": is_migration_source(&address),
                        "reconnect_required": false,
                    });
                }
                Ok(PinMigration::PreservedCustom { address }) => {
                    let pin = load_server_pin(&state.state_dir).expect("pin exists");
                    response.payload = json!({
                        "migrated": false,
                        "address": address.clone(),
                ...
                Err(crate::PinMigrationError::InvalidAddress) => {
                    response.error = Some(ProtocolError {
                        code: "invalid_request".into(),
                        ui_key: "error.server.invalidAddress".into(),
                        retryable: false,
                        detail: "migration destination must be a host and non-zero port".into(),
                    });
                }
                Err(crate::PinMigrationError::NotConfigured) => {
                    response.error = Some(ProtocolError {
                        code: "server_unconfigured".into(),
                        ui_key: "error.server.unconfigured".into(),
                        retryable: false,
                        detail: "no server pin is configured".into(),
                    });
                }
            }
        }
        ...
        "pairing.create" | "pairing.submit" | "pairing.invite" | "pairing.incoming"
        | "pairing.accept" | "pairing.decline" | "pairing.cancel" | "pairing.status" => {
...
fn start_call_inner(state: &mut CoreState, response: &mut Envelope, takeover: bool) {
    ...
    let media = match MediaSupervisor::start_with_executable(
        state.media_worker_path.as_deref(),
        state.media_wake.clone(),
    ) {
        Ok(media) => media,
        Err(error) => {
    ...
    let mut media_payload = audio_call_payload(state, &call_id);
    let turn_credentials = signaling::turn_credentials(state);
    media_payload["turn_servers"] = turn_ice_servers(state, turn_credentials);
    match media.request("call.start", media_payload) {
        Ok(reply) if reply.error.is_none() && reply.message_type == "call.start" => {
            ...
            let dialed = if takeover {
                ...
                signaling::dial_to(
                    state,
                    &target,
                    &json!({"call_id": call_id, "signal": offer}),
                )
            };
            match dialed {
                Ok(()) => {
...
fn configure_device(state: &mut CoreState, payload: &Value, response: &mut Envelope) {
    let Some(patch) = payload.as_object() else {
        response.error = Some(device_error(
            "invalid_request",
            "error.protocol.invalidRequest",
        ));
        return;
...
fn validate_phone_notification(payload: &Value) -> Result<(), ()> {
    ...
    if app.trim().is_empty()
        || (title.trim().is_empty() && text.trim().is_empty())
        || app.len() > 256
        || title.len() > 512
        || text.len() > 4096
        || timestamp == 0
...
fn forward_phone_notification(state: &mut CoreState, payload: &Value, response: &mut Envelope) {
    if !state
        .settings
        .as_ref()
        .is_some_and(|settings| settings.values().share_phone_notifications)
        || validate_phone_notification(payload).is_err()
    {
        response.error = Some(device_error(
            "notification_unavailable",
            "error.mobile.notificationsUnavailable",
        ));
        return;
    }
...
fn accept_incoming_call(state: &mut CoreState, response: &mut Envelope) {
    ...
    let turn_credentials = signaling::turn_credentials(state);
    payload["turn_servers"] = turn_ice_servers(state, turn_credentials);
...
fn audio_call_payload(state: &CoreState, call_id: &str) -> Value {
    ...
    // ICE servers ride the same call payload: the STUN address is the server
    // pin host with the same port over UDP (STUN-lite shares the listener's
    // address). Absent without a configured server — the worker then keeps
    // its current host-candidates-only behavior.
    let stun_servers: Vec<String> = load_server_pin(&state.state_dir)
        .map(|pin| format!("stun:{}", pin.address))
        .into_iter()
        .collect();
    json!({
        "call_id": call_id,
...
fn turn_ice_servers(state: &CoreState, credentials: Option<Value>) -> Value {
    ...
    json!([{
        "urls": [format!("turn:{}?transport=udp", pin.address)],
...
fn list_audio_devices(state: &mut CoreState, response: &mut Envelope) {
    ...
    let media = if let Some(media) = state.media.as_ref() {
        ...
        temporary = match MediaSupervisor::start_with_executable(
            state.media_worker_path.as_deref(),
            state.media_wake.clone(),
        ) {
            Ok(media) => media,
...
fn mic_test_start(state: &mut CoreState, payload: &Value, response: &mut Envelope) {
    ...
    if state.mic_test.is_none() {
        match MediaSupervisor::start_with_executable(
            state.media_worker_path.as_deref(),
            state.media_wake.clone(),
        ) {
...
fn accept_transfer(
    ...
    // The worker path owns accept/reject inside calls; outside them the
    // relay carries the verdict to the peer waiting on it.
    if !state.worker_covers_files() {
        let kind = if accepted {
            KIND_FILE_ACCEPT
        } else {
            KIND_FILE_REJECT
        };
        state.relay_send_frame(kind, json!({"transfer_id": id}));
    }
    state.direct_dirty = true;
    response.payload = state.direct_payload();
}
...
fn cancel_transfer(state: &mut CoreState, payload: &Value, response: &mut Envelope) {
    ...
    if !state.worker_covers_files() {
        state.relay_send_frame(KIND_FILE_CANCEL, json!({"transfer_id": id}));
    }
    state.direct_dirty = true;
    response.payload = state.direct_payload();
}
...
fn run_network_diagnostics(state: &mut CoreState) -> Result<Value, ProtocolError> {
    ...
    let mut direct = match state.call.stats.as_ref() {
    ...
    // Direct-path candidates the next dial would race: local source addresses
    // plus the STUN reflexive, if the server answers it. No peer context
    // here — this reports what *we* can offer, measured live. The same
    // probe times the UDP path: one socket on the server serves STUN and
    // TURN alike, so a answered binding is the relay/TURN datagram path
    // measured, not promised.
    let udp_started = Instant::now();
    let reflexive = mobile_link::stun_reflexive(&pin.address, Duration::from_millis(1500));
    let udp_rtt_ms = reflexive
        .as_ref()
        .map(|_| udp_started.elapsed().as_secs_f64() * 1000.0);
    let udp = json!({
        "reachable": reflexive.is_some(),
        "rtt_ms": udp_rtt_ms,
    });
    if let Some(object) = direct.as_object_mut() {
        object.insert(
            "candidates".to_owned(),
            json!(mobile_link::local_dial_addrs(&pin.address)),
        );
        object.insert(
            "reflexive".to_owned(),
            json!(reflexive.map(|addr| addr.to_string())),
        );
    }

    // Relay fallback: honest client-side state only — a ready session the
    // bearer would use right now. No server internals, no addresses.
    let relay = json!({
        "active": !state.relay.ready_peers().is_empty(),
    });
...
fn list_paired_contacts(state: &mut CoreState) -> Result<Value, ProtocolError> {
    ...
    let mut client = crate::ServerClient::connect(&pin).map_err(|_| server_unavailable_error())?;
    let request = Envelope::request("contacts.list", json!({}), crate::rfc3339_now());
    let response = client
...
fn run_pairing(
    ...
    let run = |session: &mut PairingSession| -> Result<Value, PairingError> {
        match message_type {
            ...
            "pairing.invite" => {
                // Harbor-ID invitation: the peer is named by full Harbor ID.
                // Accepts "peer" (new UIs) or falls back to nothing — a
                // missing peer is a local validation failure, never a guess.
                let peer = payload.get("peer").and_then(Value::as_str).unwrap_or("");
                session
                    .peer_connect(&pin, &identity, peer)
                    .map(|_| session.snapshot())
            }
...
fn pairing_protocol_error(session: &mut PairingSession, error: PairingError) -> ProtocolError {
    let (code, ui_key, retryable, detail) = match error {
        ...
        // Local Harbor-ID validation failed before any network I/O: the UI
        // normally prevents this (disabled Connect), so this is defense in
        // depth with a non-technical key the UI already knows.
        PairingError::InvalidHarborId => (
            "invalid_request".to_owned(),
            "error.pairing.invalidHarborId".to_owned(),
            false,
            "The Harbor ID must look like harbor-xxxxxxxx (8 hex characters)".to_owned(),
        ),
...
mod tests {
    ...
    fn phone_notifications_are_bounded_ephemeral_events() {
        ...
        assert!(
            validate_phone_notification(&json!({
    ...
    fn presence_senses_commit_into_events_and_snapshots_stay_private() {
        ...
        assert!(
            !serialized.contains("inputIdle"),
            "evidence leaked: {serialized}"
        );
        assert!(
            !serialized.contains("sessionActive"),
            "evidence leaked: {serialized}"
        );
    ...
    fn paired_profiles_converge_peer_to_peer() {
        let a_dir = std::env::temp_dir().join(format!("harbor-profile-a-{}", uuid::Uuid::new_v4()));
        let b_dir = std::env::temp_dir().join(format!("harbor-profile-b-{}", uuid::Uuid::new_v4()));
    ...
    fn harbor_id_of(state: &mut CoreState) -> String {
        dispatch(
            Envelope::request("identity.get", json!({}), "2026-08-31T20:00:00Z"),
            state,
        )
    ...
    #[test]
    fn pairing_invite_validates_locally_before_touching_the_network() {
        use harbor_control::is_harbor_id;
        let directory =
            std::env::temp_dir().join(format!("harbor-core-invite-local-{}", Uuid::new_v4()));
        let mut state = CoreState::for_directory(&directory);
        let fingerprint = "e".repeat(64);
        // Point at a dead port: anything reaching the network fails fast.
        dispatch(
            Envelope::request(
                "server.configure",
                json!({"address": "127.0.0.1:1", "fingerprint": fingerprint}),
                "2026-08-31T20:00:00Z",
            ),
            &mut state,
        )
        .unwrap();
        // The full user matrix: malformed IDs die locally with invalid_request
        // (no request is sent; nothing to retry against the network).
        for bad in [
            "123456",
            "d31846b8",
            "harbor-d31846b",
            ...
            assert_eq!(error.ui_key, "error.pairing.invalidHarborId");
        }
        // A well-formed but unreachable peer passes validation and fails on
        // connect (server_unavailable, retryable) — never as invalid input.
        let refused = dispatch(
            Envelope::request(
                "pairing.invite",
                json!({"peer": "harbor-d31846b8"}),
                "2026-08-31T20:00:03Z",
            ),
            &mut state,
        )
        .unwrap();
        assert_eq!(refused.error.unwrap().code, "server_unavailable");
        std::fs::remove_dir_all(directory).unwrap();
    }

    #[test]
    fn pairing_invite_flows_end_to_end_with_full_harbor_ids() {
        use std::sync::{Arc, Mutex};

        use harbor_server::{Listener, ListenerConfig};

        let server_dir =
            std::env::temp_dir().join(format!("harbor-core-invite-server-{}", Uuid::new_v4()));
        ...
        let first_dir =
            std::env::temp_dir().join(format!("harbor-core-invite-first-{}", Uuid::new_v4()));
        let second_dir =
            std::env::temp_dir().join(format!("harbor-core-invite-second-{}", Uuid::new_v4()));
        let mut first = CoreState::for_directory(&first_dir);
        let mut second = CoreState::for_directory(&second_dir);
        ...
        // Both Harbor IDs are minted locally, canonical shape, distinct.
        let first_harbor = harbor_id_of(&mut first);
        let second_harbor = harbor_id_of(&mut second);
        assert!(harbor_control::is_harbor_id(&first_harbor));
        assert!(harbor_control::is_harbor_id(&second_harbor));
        assert_ne!(first_harbor, second_harbor);

        // Unknown and self invites are refused; nothing is stored.
        // (Second registers itself first via an empty incoming poll —
        // registration needs its private key, so only its own device can do
        // it; inviting a never-seen device stays "unavailable".)
        let pre = dispatch(
            Envelope::request("pairing.incoming", json!({}), crate::rfc3339_now()),
            &mut second,
        )
        .unwrap();
        assert_eq!(pre.payload["has_request"], false);
        for (peer, code) in [
            ("harbor-cccccccc", "pairing_refused"),
            (first_harbor.as_str(), "pairing_refused"),
        ] {
            let refused = dispatch(
                Envelope::request(
                    "pairing.invite",
        ...
        // Invite by full Harbor ID: the exact value echoes back verbatim.
        let invited = dispatch(
            Envelope::request(
                "pairing.invite",
                json!({"peer": second_harbor}),
                crate::rfc3339_now(),
            ),
            &mut first,
        )
        ...
        assert_eq!(invited.payload["phase"], "REQUESTING");
        assert_eq!(invited.payload["peer_harbor_id"], second_harbor);
        let pairing_id = invited.payload["pairing_id"].as_str().unwrap().to_owned();

        // Retry while live returns the same invitation (no duplicates).
        let retry = dispatch(
            Envelope::request(
                "pairing.invite",
                json!({"peer": second_harbor}),
        ...
        .unwrap();
        assert_eq!(retry.payload["pairing_id"].as_str().unwrap(), pairing_id);

        // The target sees who asks, by Harbor ID — never a bare UUID.
        let polled = dispatch(
            Envelope::request("pairing.incoming", json!({}), crate::rfc3339_now()),
            &mut second,
        )
        ...
        let request = polled.payload["request"].clone();
        assert_eq!(request["pairing_id"], pairing_id);
        assert_eq!(request["requester_harbor_id"], first_harbor);

        // Accept, then both sides observe the relationship.
        ...
        assert_eq!(peers[0]["harbor_id"], second_harbor);

        // Restart persistence: the relationship and both identities survive;
        // pairing changed neither local identity.
        drop(first);
        drop(second);
        let mut first = CoreState::for_directory(&first_dir);
        let mut second = CoreState::for_directory(&second_dir);
        assert_eq!(harbor_id_of(&mut first), first_harbor);
        assert_eq!(harbor_id_of(&mut second), second_harbor);
        let contacts = dispatch(
            Envelope::request("contacts.list", json!({}), crate::rfc3339_now()),
            &mut first,
        )
        .unwrap();
        assert_eq!(contacts.payload["peers"].as_array().unwrap().len(), 1);
    ...
    fn server_pin_configuration_validates_persists_and_round_trips() {
        ...
        // A non-legacy address never needs migration (configure reports it).
        assert_eq!(configured.payload["needs_migration"], false);
        std::fs::remove_dir_all(directory).unwrap();
    }

    #[test]
    fn server_migrate_moves_legacy_preserves_custom_and_stays_idempotent() {
        use crate::{LEGACY_K11_ENDPOINTS, is_migration_source};
        // TEST-NET-1 stands in for the operator-configured Oracle endpoint:
        // the real address arrives via HARBOR_ORACLE_ADDRESS, never compiled.
        const TEST_ORACLE: &str = "192.0.2.99:9091";
        let directory =
            std::env::temp_dir().join(format!("harbor-core-migrate-{}", Uuid::new_v4()));
        let mut state = CoreState::for_directory(&directory);
        // No pin: migrate refuses with server_unconfigured (fresh installs
        // configure directly instead).
        let empty = dispatch(
            Envelope::request("server.migrate", json!({}), "2026-08-31T20:00:00Z"),
            &mut state,
        )
        .unwrap();
        assert_eq!(empty.error.unwrap().code, "server_unconfigured");
        // Legacy K11 pin reports needs_migration.
        let fingerprint = "c".repeat(64);
        dispatch(
            Envelope::request(
                "server.configure",
                json!({"address": LEGACY_K11_ENDPOINTS[0], "fingerprint": fingerprint}),
                "2026-08-31T20:00:01Z",
            ),
            &mut state,
        )
        .unwrap();
        let config = dispatch(
            Envelope::request("server.config", json!({}), "2026-08-31T20:00:02Z"),
            &mut state,
        )
        .unwrap();
        assert_eq!(config.payload["needs_migration"], true);
        // Migrate once: fingerprint preserved, reconnect flagged.
        let migrated = dispatch(
            Envelope::request(
                "server.migrate",
                json!({"to_address": TEST_ORACLE}),
                "2026-08-31T20:00:03Z",
            ),
            &mut state,
        )
        ...
        assert_eq!(migrated.payload["migrated"], true);
        assert_eq!(migrated.payload["from"], LEGACY_K11_ENDPOINTS[0]);
        assert_eq!(migrated.payload["address"], TEST_ORACLE);
        assert_eq!(migrated.payload["fingerprint"], fingerprint);
        assert_eq!(migrated.payload["reconnect_required"], true);
        // Second run: no rewrite, no reconnect.
        let again = dispatch(
            Envelope::request(
                "server.migrate",
                json!({"to_address": TEST_ORACLE}),
        ...
        .unwrap();
        assert_eq!(again.payload["migrated"], false);
        assert_eq!(again.payload["reconnect_required"], false);
        // Custom endpoints are never overwritten.
        dispatch(
            Envelope::request(
                "server.configure",
                json!({"address": "custom.example.com:9091", "fingerprint": fingerprint}),
        ...
        .unwrap();
        assert_eq!(preserved.payload["migrated"], false);
        assert_eq!(preserved.payload["preserved_custom"], true);
        assert_eq!(preserved.payload["address"], "custom.example.com:9091");
        // Invalid destinations are refused without touching the pin.
        let bad = dispatch(
            Envelope::request(
                "server.migrate",
                json!({"to_address": "not an address!!"}),
        ...
        assert!(is_migration_source(LEGACY_K11_ENDPOINTS[0]));
        assert!(!is_migration_source("custom.example.com:9091"));
        std::fs::remove_dir_all(directory).unwrap();
    }

    #[test]
    fn server_migrate_env_oracle_endpoint_covers_both_hops() {
        // The operator address travels via HARBOR_ORACLE_ADDRESS end to end:
        // default destination, migration source for the second hop, and the
        // server.config migration_target. Save/restore around the test; no
        // other test in this binary touches these variables.
        const TEST_ORACLE: &str = "192.0.2.99:9091";
        // Save/restore the operator variables even on assertion panic; no
        // other test in this binary touches them, but a leaked value would
        // still change server.migrate defaults process-wide.
        struct EnvRestore {
        ...
        let directory =
            std::env::temp_dir().join(format!("harbor-core-migrate-env-{}", Uuid::new_v4()));
        let mut state = CoreState::for_directory(&directory);
        let fingerprint = "d".repeat(64);
        dispatch(
            Envelope::request(
                "server.configure",
                json!({"address": crate::LEGACY_K11_ENDPOINTS[0], "fingerprint": fingerprint}),
                "2026-08-31T20:00:01Z",
            ),
            &mut state,
        )
        .unwrap();
        // No env and no explicit destination: refused, pin untouched.
        let missing = dispatch(
            Envelope::request("server.migrate", json!({}), "2026-08-31T20:00:02Z"),
            &mut state,
        )
        .unwrap();
        ...
        .unwrap();
        assert_eq!(config.payload["migration_target"], "");
        // With env set, bare migrate follows it (first hop, no arg needed).
        unsafe {
            std::env::set_var("HARBOR_ORACLE_ADDRESS", TEST_ORACLE);
        }
        let first = dispatch(
            Envelope::request("server.migrate", json!({}), "2026-08-31T20:00:04Z"),
            &mut state,
        )
        .unwrap();
        assert!(first.error.is_none(), "{:?}", first.error);
        assert_eq!(first.payload["migrated"], true);
        assert_eq!(first.payload["address"], TEST_ORACLE);
        assert_eq!(first.payload["fingerprint"], fingerprint);
        // The env address is itself a source: second hop to a later address.
        let second = dispatch(
            Envelope::request(
                "server.migrate",
                json!({"to_address": "harbor.example.com:9091"}),
                "2026-08-31T20:00:05Z",
            ),
            &mut state,
        )
        ...
        assert_eq!(second.payload["migrated"], true);
        assert_eq!(second.payload["from"], TEST_ORACLE);
        assert_eq!(second.payload["address"], "harbor.example.com:9091");
        std::fs::remove_dir_all(directory).unwrap();
    }
    ...
    /// Relay chat end to end through the rig server with no direct bearer:
    /// host opens on queued chat, peer accepts over polls plus key signals,
    /// frames seal and unseal end to end, and the transcript converges — all
    /// without an external overlay, TUN, listeners, or hand-driven sockets.
    #[test]
    fn relay_chat_flows_end_to_end_without_direct_bearer() {
        let (listener, mut host, mut peer, server_dir, host_dir, peer_dir) =
            spawn_server_and_pair();
        // Peer is the mobile leg; host stays desktop.
        peer.settings
            .as_mut()
            .unwrap()
        ...
        // Signaling rounds each way: legs, pair sessions, device hellos.
        for _ in 0..3 {
            let _ = signaling::tick(&mut host);
            let _ = signaling::tick(&mut peer);
        }
        assert!(
            host.signaling
                .legs()
                .iter()
                .any(|leg| leg.device == Some(DeviceType::Mobile)),
            "host must learn the mobile leg before relay can trigger"
        );

        // Queued chat with no bearer and no worker path opens a session.
        let queued = dispatch(
            Envelope::request(
                "chat.send",
                json!({"body": "relay hello"}),
                crate::rfc3339_now(),
            ),
            &mut host,
        )
        ...
        assert!(
            host.relay.sessions_snapshot().iter().any(|(peer, _)| host
                .signaling
                .legs()
                .iter()
                .any(|leg| &leg.peer == peer && leg.device == Some(DeviceType::Mobile))),
            "an opening session toward the mobile leg must exist"
        );

        // Peer learns the keys signal, stages and accepts, and answers keys.
        let _ = signaling::tick(&mut peer);
        assert!(!peer.pending_relay_keys.is_empty(), "keys signal must land");
        peer.pump_relay();
        // Host learns the peer half and completes.
        let _ = signaling::tick(&mut host);
        host.pump_relay();
        assert!(
            !host.relay.ready_peers().is_empty(),
            "opener must complete once keys arrive"
        );

        // Flush travels sealed over the control connection; the parked poll
        // delivers within a loop tick.
        host.sync_direct();
        assert_eq!(host.chat.queue_len(), 0, "sealed chat must leave the queue");
        std::thread::sleep(std::time::Duration::from_millis(500));
        peer.pump_relay();
        let transcript = peer.chat.snapshot();
        assert!(
            transcript
                .iter()
                .any(|message| message.body == "relay hello"),
            "peer transcript must converge over the relay"
        );

        // Reverse direction: the peer's ack returns sealed, and the host
        // marks Delivered — the pre-completion inbox race once broke exactly
        // this path while delivery looked fine.
        peer.pump_relay();
        ...
        assert!(
            ...
            "host must observe the relayed ack"
        );

        peer.relay_worker.stop();
        host.relay_worker.stop();
        cleanup_e2e(listener, &[server_dir, host_dir, peer_dir]);
    }

    /// Drives two rig cores through a complete relay handshake (open, keys,
    /// accept, complete) with workers parked: returns with sessions Ready on
    /// both sides. Legs must already be learned (three tick rounds each way).
    /// Queues one chat on the host to trigger the open; it flows harmlessly
    /// alongside file traffic and proves coexistence.
    fn establish_relay(host: &mut CoreState, peer: &mut CoreState) {
        let queued = dispatch(
            Envelope::request(
                "chat.send",
                json!({"body": "relay up"}),
        ...
        host.pump_relay();
        assert!(
            !host.relay.sessions_snapshot().is_empty(),
            "open must stage a session"
        );
        let _ = signaling::tick(peer);
        assert!(!peer.pending_relay_keys.is_empty(), "keys must land");
        peer.pump_relay();
        let _ = signaling::tick(host);
        host.pump_relay();
        assert!(
            !host.relay.ready_peers().is_empty(),
            "opener must complete once keys arrive"
        );
        assert_eq!(peer.relay.sessions_snapshot().len(), 1);
        assert_eq!(peer.relay.ready_peers().len(), 1);
    }

    /// Relay file transfer end to end with no direct bearer: offer, accept,
    /// chunks, SHA-verified finalize, and receipt — sealed throughout.
    /// Sized at 10 MiB: the full machinery (640 chunks) well past unit
    /// scale, fast enough to stay in-suite. The 100 MiB plan gate is the
    /// same code path (linear scaling, proven 1→10 MiB); run it manually
    /// per `docs/network-plan.md` — in this sandbox each exchange costs
    /// milliseconds (scheduler/timer granularity), so 100 MiB needs minutes
    /// here and seconds-to-minutes on real links (stop-and-wait bound).
    #[test]
    fn relay_file_transfer_completes_end_to_end() {
        const FILE_BYTES: usize = 10 << 20;
        let (listener, mut host, mut peer, server_dir, host_dir, peer_dir) =
            spawn_server_and_pair();
        peer.settings
            .as_mut()
        ...
        host.relay.set_throttle_override(Some(u64::MAX));
        peer.relay.set_throttle_override(Some(u64::MAX));

        let source: Vec<u8> = (0..FILE_BYTES).map(|i| (i % 251) as u8).collect();
        let source_path = host_dir.join("source.bin");
        ...
        let transfer_id = offered.payload["transfer_id"].as_str().unwrap().to_owned();

        establish_relay(&mut host, &mut peer);

        // Peer learns the relayed offer and accepts (staging first).
        let mut accepted = false;
        for _ in 0..200 {
            std::thread::sleep(std::time::Duration::from_millis(50));
            peer.pump_relay();
            if peer.transfers.records().iter().any(|r| r.id == transfer_id) {
                let accept = dispatch(
                    Envelope::request(
                        "transfer.accept",
                ...
                .unwrap();
                assert!(accept.error.is_none(), "{accept:?}");
                accepted = true;
                break;
            }
        }
        assert!(accepted, "peer must see the relayed offer");

        // Pump chunks (batched host pumps per worker round) until finalize.
        let mut done = false;
        for _ in 0..2000 {
            for _ in 0..10 {
                host.pump_relay();
            ...
            {
                if record.phase == TransferPhase::Completed {
                    done = true;
                    break;
                }
            }
        }
        assert!(done, "peer must finalize the relayed file");

        // Byte-identical landing, then receipt back to the sender.
        let landed = std::fs::read(peer_dir.join("downloads").join("source.bin")).unwrap();
        assert_eq!(landed, source);
        let mut received = false;
        for _ in 0..200 {
            std::thread::sleep(std::time::Duration::from_millis(25));
            // Both app cores keep pumping in production. The receiver must
            // run so a temporarily refused receipt can leave its retry queue.
            let _ = signaling::tick(&mut peer);
            let _ = signaling::tick(&mut host);
            peer.pump_relay();
            host.pump_relay();
        ...
        assert!(
            received,
            "sender must observe peer receipt (pending={:?}, peer_sessions={}, host_sessions={})",
            peer.pending_relay_receipts,
            peer.relay.sessions_snapshot().len(),
            host.relay.sessions_snapshot().len(),
        );
    ...
    /// A rejected relayed offer surfaces as canceled on the sender: the
    /// file_reject frame travels the same sealed path as the offer.
    #[test]
    fn relay_file_reject_cancels_the_sender_offer() {
        let (listener, mut host, mut peer, server_dir, host_dir, peer_dir) =
            spawn_server_and_pair();
        peer.settings
            .as_mut()
        ...
        let source_path = host_dir.join("rejected.bin");
        ...
        establish_relay(&mut host, &mut peer);
        // Offer reaches the peer; reject it there.
        let mut seen = false;
        for _ in 0..200 {
            std::thread::sleep(std::time::Duration::from_millis(50));
            peer.pump_relay();
        ...
        assert!(seen, "peer must see the relayed offer");
        // The reject returns sealed; the sender marks the offer canceled.
        ...
        for _ in 0..200 {
            ...
            if let Some(record) = host
                ...
                .find(|r| r.id == transfer_id)
            {
                if record.phase == TransferPhase::Canceled {
                    canceled = true;
                    break;
                }
            }
        }
        assert!(canceled, "sender must observe the relayed reject");
    ...
    /// A sender-side cancel mid-transfer reaches the receiver sealed: the
    /// receiver observes it instead of waiting out the offer TTL.
    #[test]
    fn relay_file_cancel_reaches_an_active_receiver() {
        let (listener, mut host, mut peer, server_dir, host_dir, peer_dir) =
            spawn_server_and_pair();
        peer.settings
            .as_mut()
        ...
        let source_path = host_dir.join("canceled.bin");
        std::fs::write(&source_path, vec![7_u8; 1 << 16]).unwrap();
        let offered = dispatch(
            Envelope::request(
                "transfer.offer_local",
                json!({"source_path": source_path}),
        ...
        // Cancel on the sender before any chunk lands; the receiver observes.
        ...
        .unwrap();
        assert!(canceled.error.is_none(), "{canceled:?}");
        ...
        assert!(observed, "receiver must observe the relayed cancel");
    ...
    /// A queue refusal after sealing retires the session: queued chat reopens
    /// with a fresh id and ephemeral key instead of reusing an AEAD nonce.
    #[test]
    fn relay_busy_retires_session_and_reopens_with_fresh_identity() {
        let (listener, mut host, mut peer, server_dir, host_dir, peer_dir) =
            spawn_server_and_pair();
        peer.settings
            .as_mut()
        ...
        peer.relay_worker.stop();
        host.relay.set_throttle_override(Some(u64::MAX));
        let (relay_peer, first) = host.relay.sessions_snapshot()[0];
        let first_key = host.relay.session_for_id(&first).unwrap().eph_pub;
        let queued_before = host.chat.queue_len();
        let transcript_before: Vec<_> = host
            .chat
            .snapshot()
            .into_iter()
            .map(|message| (message.id, message.body, message.delivery))
            .collect();
        let (message_id, message_body, initial_delivery) = transcript_before
            .iter()
            .find(|(_, _, delivery)| *delivery == Delivery::WaitingForConnection)
            .cloned()
            .expect("establishing the relay leaves its trigger chat queued");

        let mut refused = false;
        for index in 0..16 {
            if !host.relay_send_frame_to(relay_peer, "busy_probe", json!({"index": index})) {
                refused = true;
                break;
            }
        }
        assert!(
            refused,
            "the bounded server queue must refuse a sealed frame"
        );
        assert!(
            host.relay.sessions_snapshot().is_empty(),
            "relay_busy must retire the local session"
        );
        assert_eq!(host.chat.queue_len(), queued_before);
        assert_eq!(
            host.chat
                .snapshot()
                .into_iter()
        ...
        host.pump_relay();
        let (_, second) = host.relay.sessions_snapshot()[0];
        let second_key = host.relay.session_for_id(&second).unwrap().eph_pub;
        assert_ne!(first, second, "reopen must mint a new session identity");
        assert_ne!(
            first_key, second_key,
            "reopen must mint fresh ephemeral keys"
        );
        assert_eq!(host.chat.queue_len(), queued_before);
        assert_eq!(
            host.chat
        ...
        host.note_relay_open(first, relay_peer);
        host.pending_relay_keys.push((first, relay_peer, first_key));
        host.consume_relay_keys();
        assert_eq!(
            host.relay.session_for_peer(&relay_peer).unwrap().relay_id,
            second,
            "stale open and key events must preserve the replacement session"
        );

        let mut replacement_staged = false;
        for _ in 0..100 {
            std::thread::sleep(std::time::Duration::from_millis(25));
            peer.pump_relay();
            if peer
                ...
                .sessions_snapshot()
                .iter()
                .any(|(_, relay_id)| *relay_id == second)
            {
                replacement_staged = true;
                break;
            }
        }
        assert!(replacement_staged, "peer must observe the replacement open");
        ...
        for _ in 0..100 {
            ...
            let _ = signaling::tick(&mut host);
            host.pump_relay();
            if !host.relay.ready_peers().is_empty() && !peer.relay.ready_peers().is_empty() {
                replacement_ready = true;
                break;
            }
            std::thread::sleep(std::time::Duration::from_millis(25));
        }
        assert!(replacement_ready, "replacement handshake must complete");

        assert_eq!(initial_delivery, Delivery::WaitingForConnection);
        host.sync_direct();
        assert_eq!(host.chat.queue_len(), 0);
        let mut delivered = false;
        for _ in 0..100 {
            std::thread::sleep(std::time::Duration::from_millis(25));
            peer.pump_relay();
            host.pump_relay();
            let peer_has_message = peer.chat.snapshot().iter().any(|message| {
                message.id == message_id
                    && message.body == message_body
                    && message.direction == Direction::Incoming
                    && message.delivery == Delivery::Delivered
            });
            let host_has_ack = host.chat.snapshot().iter().any(|message| {
                message.id == message_id
                    && message.body == message_body
                    && message.direction == Direction::Outgoing
                    && message.delivery == Delivery::Delivered
            });
            if peer_has_message && host_has_ack {
                delivered = true;
                break;
            }
        }
        assert!(
            delivered,
            "queued chat must deliver after relay_busy recovery"
        );
        assert_eq!(
            peer.chat
                .snapshot()
    ...
    fn companion_rig_pairs_and_chats_through_the_deployed_server() {
        ...
        let rig_dir = std::env::temp_dir().join(format!("harbor-rig-taylor-{}", Uuid::new_v4()));
        ...
        assert!(
            accepted,
            "the phone never submitted the code -- pair it first"
        );
            ...
            if last_port_print.elapsed() > Duration::from_secs(15) {
                ...
                if let Ok(snapshot) = taylor.device_snapshot() {
                    eprintln!(
                        ...
                        snapshot["linkListeningPort"], snapshot["linkPeer"], snapshot["peers"]
                    );
                }
            }
...
"#;
    ...
    fn share_commands_round_trip_through_the_worker_boundary() {
        ...
        let started = dispatch(
            Envelope::request("call.share_screen_start", json!({}), crate::rfc3339_now()),
            &mut core,
        )
        .unwrap();
        ...
        let stopped = dispatch(
            Envelope::request("call.share_screen_stop", json!({}), crate::rfc3339_now()),
            &mut core,
        )
        .unwrap();
    ...
    fn transfer_offers_follow_local_policy_before_any_transport() {
        ...
        let invalid = dispatch(
            Envelope::request("transfer.offer_local", json!({}), crate::rfc3339_now()),
            &mut core,
        )
        .unwrap();
        assert_eq!(
        ...
        for kind in ["transfer.accept", "transfer.reject", "transfer.cancel"] {
            let unknown = dispatch(
                Envelope::request(kind, json!({"transfer_id": "nope"}), crate::rfc3339_now()),
                &mut core,
            )
            .unwrap();
    ...
    fn diagnostics_report_an_unreachable_server_honestly() {
        ...
        // The UDP probe and the relay state report honestly even when the
        // control plane is down: unreachable and idle, never invented.
        assert_eq!(response.payload["udp"]["reachable"], false);
        assert!(response.payload["udp"]["rtt_ms"].is_null());
        assert_eq!(response.payload["relay"]["active"], false);
    }
    ...
    fn diagnostics_measure_a_real_pinned_exchange() {
        ...
        // The direct report always carries what the next dial would race:
        // local candidates (loopback is unconditional) and the STUN
        // reflexive — null here, the rig server speaks TCP only.
        let candidates = response.payload["direct"]["candidates"]
            .as_array()
            .expect("candidates travel with every configured report");
        assert!(
            candidates.iter().any(|addr| addr == "127.0.0.1"),
            "{candidates:?}"
        );
        assert!(response.payload["direct"]["reflexive"].is_null());

        // The same socket serves STUN and TURN: lighting up the server's
        // UDP side must turn the relay/UDP card measured on the next run.
        let stun = harbor_server::StunServer::spawn(
            listener.local_addr(),
            std::sync::Arc::new(std::sync::Mutex::new(harbor_server::TurnState::new())),
        )
        .unwrap();
        let response = dispatch(
            Envelope::request("network.diagnostics", json!({}), crate::rfc3339_now()),
        ...
        .unwrap();
        assert!(response.error.is_none(), "{:?}", response.error);
        assert_eq!(response.payload["udp"]["reachable"], true);
        let udp_rtt = response.payload["udp"]["rtt_ms"].as_f64().unwrap();
        assert!(udp_rtt > 0.0, "a real UDP echo takes measurable time");
        assert!(!response.payload["direct"]["reflexive"].is_null());
        assert_eq!(response.payload["relay"]["active"], false);
        stun.shutdown();
    ...
    #[test]
    fn audio_call_payload_carries_stun_servers_from_the_pin() {
        let mut core_state = state();
        // No server configured: no STUN address to advertise; the worker
        // keeps host-candidates-only behavior.
        assert_eq!(
            audio_call_payload(&core_state, "call-9")["stun_servers"],
            json!([])
        );
        ...
        let configured = dispatch(
            Envelope::request(
                "server.configure",
                json!({"address": "[2001:db8::1]:9091", "fingerprint": "ab".repeat(32)}),
                crate::rfc3339_now(),
            ),
            &mut core_state,
        )
        .unwrap();
        assert!(configured.error.is_none(), "{configured:?}");
        // The STUN URL is the pin host with the same port over UDP: bracketed
        // IPv6 survives verbatim, no address parsing required.
        assert_eq!(
            audio_call_payload(&core_state, "call-9")["stun_servers"],
            json!(["stun:[2001:db8::1]:9091"])
        );
    }
    ...
    fn device_state_reports_this_install_as_desktop_standalone() {
        ...
        assert!(
            !response.payload["device"]["deviceId"]
                .as_str()
                .unwrap()
                .is_empty()
        );
    ...
    fn device_state_names_peers_blocked_and_link() {
        ...
        core_state
            .signaling
            .note_peer_device(&peer, DeviceType::Mobile);
        let response = dispatch(
            Envelope::request("device.state", json!({}), "2026-08-31T20:00:00Z"),
            &mut core_state,
        )
    ...
    fn link_pair_exchanges_chat_profile_and_phone_state() {
        ...
        let mob_texts: Vec<String> = mobile
            ...
            .collect();
        assert!(
            desk_texts.iter().any(|body| body == "oi do mobile"),
            "{desk_texts:?}"
        );
        assert!(
            desk_texts.iter().any(|body| body == "oi do desktop"),
            "{desk_texts:?}"
        );
        assert!(
            mob_texts.iter().any(|body| body == "oi do mobile"),
            "{mob_texts:?}"
        );
        assert!(
            mob_texts.iter().any(|body| body == "oi do desktop"),
            "{mob_texts:?}"
        );
    ...
    fn abi_dispatch_answers_device_state() {
        let request =
            harbor_protocol::Envelope::request("device.state", json!({}), "2026-08-31T20:00:00Z");
    ...
    fn abi_rejects_garbage_and_nulls() {
        let mut out_len = 0_usize;
        assert!(
            harbor_core_dispatch(std::ptr::null_mut(), [0_u8; 4].as_ptr(), 4, &mut out_len)
                .is_null()
        );
...
pub extern "C" fn harbor_core_tick(handle: *mut CoreState, out_len: *mut usize) -> *mut c_uchar {
    ...
    if state.call.call_id.is_some()
        ...
        && state
            .media
            .as_ref()
            .is_some_and(|media| !media.is_running())
    {
        state.fail_media();
    }
