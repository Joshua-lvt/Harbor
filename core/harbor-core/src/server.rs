...
impl ServerClient {
    ...
    pub fn connect(pin: &ServerPin) -> Result<Self, ServerClientError> {
        ...
        // Relay long-polls hold for 25 s, so I/O must remain above that while
        // still bounding a TLS peer that accepts and then stalls forever.
...
mod tests {
    ...
    fn pinned_clients_exchange_signed_requests_with_the_real_server() {
        ...
        let registered = client
            .exchange(
                identity_update(device_id, &local.record().harbor_id, &public_key),
                &local,
            )
            .unwrap();
