...
The signature is Ed25519 over the deterministic serialization of `signer_id` plus the envelope, encoded as unpadded Base64. Registration (`identity.update`) requires proof of possession: the payload's `device_id` must equal `signer_id` and the signature must verify against the submitted public key. A Harbor ID must have the canonical full shape, is unique under ASCII-insensitive hex casing, and is immutable for a device after first registration; same-device re-registration is idempotent. Every later request is verified against the registered public key of `signer_id`; an unknown signer or a bad signature is answered with `unauthorized` and never reaches the state machine.

## Initial message families

- Core lifecycle: `core.hello`, `core.ready`, `core.shutdown`
- Identity/settings: `identity.*`, `settings.*`
- Pairing: `pairing.create`, `pairing.submit`, `pairing.invite`, `pairing.cancel`, `pairing.incoming`, `pairing.accept`, `pairing.decline`, `pairing.status`. The current UI flow is Harbor-ID based: `pairing.invite {peer: "harbor-xxxxxxxx"}` names the peer by full Harbor ID (exact full-length match, with ASCII-insensitive hex casing, validated as `^harbor-[0-9a-fA-F]{8}$`); the six-digit code flow (`create`/`submit`) stays valid for older clients. `pairing.incoming` answers `{requests: [...]}` with each snapshot carrying `requester_harbor_id`.
...
- Relay fallback: `relay.open`, `relay.accept`, `relay.data`, `relay.poll`, `relay.close` (`relay.error` is server-originated, `relay.flow` reserved). Opaque, size-bounded frames (≤48 KiB request strings, ≤8 queued per session, ≤4 sessions) between paired devices over their existing outbound connections; verified long-polls (≤25 s hold) carry answers, delivery is pull-based, and end-to-end encryption lives on the clients — the server routes by session id and sizes frames but never interprets payloads.
- Rendezvous: `endpoint.self` — the server echoes the transport endpoint it observed for the caller's own connection (`{address, port, transport, observed_at}`). Only the currently observed value is ever served, only to the verified device it was observed for; unregistered, unverified, or transport-less callers learn nothing. Observed endpoints are transient (never persisted) and feed direct-dial candidate gathering on the client.
...
`harbor-control` owns the transport-independent pairing, relationship, presence lease, and logical-session transitions. A pairing code is exactly six ASCII digits, expires after five minutes, and authorizes peers only after the target explicitly accepts the request. A Harbor-ID invitation (`invite_pairing`) names the peer by full Harbor ID, lands directly in `WaitingApproval` (the ID itself is the introduction; code submits can never match it), retries idempotently while live, and likewise authorizes peers only after explicit acceptance. Presence is a 45-second lease and becomes `OFFLINE` after expiry. A logical session requires an accepted relationship, and SDP/ICE records are capped at 64 KiB.
...
- **Control-plane only**: the message-type allowlist has no media, screen-share, or DataChannel entries, and the 256 KiB frame cap plus 16-connection bound keep bulk transfer through the listener bounded and fenced: chat/profile/phone frames may ride the relay path (opaque, ≤48 KiB each, ≤8 queued per session, idle-expiring), while media, screen captures, and file bodies stay off the server. Signaling relay to the remote peer is the next layer, not a data path.
...
- **STUN-lite rendezvous (UDP, same port)**: alongside the TLS listener, the server answers small JSON echo datagrams (`protocol::stun` wire format, 512 B cap) on the same address over UDP, reporting the datagram's actual source. No authentication (requests carry nothing sensitive), no state, malformed datagrams silently dropped. Direct dials race IPv6/IPv4 candidates (250 ms stagger, fastest established socket wins) assembled from server-observed reflexive plus local source addresses.
