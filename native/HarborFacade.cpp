...
namespace {
// Pre-pointed Harbor network: the K11 public IPv6 endpoint, the same
// endpoint the mobile client ships with — unless this build baked an Oracle
// default (Fase 11+ releases set HARBOR_ORACLE_ADDRESS at configure time)
// or the runtime env overrides it. Public pinning material, never a secret.
const QString builtInOracleAddress()
{
#ifdef HARBOR_ORACLE_ADDRESS_BUILTIN
    return QStringLiteral(HARBOR_ORACLE_ADDRESS_BUILTIN).trimmed();
#else
    return QString();
#endif
}
...
{
    for (const char *key : {"HARBOR_SERVER_DEFAULT_ADDRESS", "HARBOR_ORACLE_ADDRESS"}) {
        const QByteArray override = qgetenv(key);
        if (!override.trimmed().isEmpty())
            return QString::fromUtf8(override.trimmed());
    }
    if (!builtInOracleAddress().isEmpty())
        return builtInOracleAddress();
    return QStringLiteral("[2804:d59:8777:ad00:3a30:f9ff:fe3e:de81]:9091");
}
    ...
    , m_appIcons(new HarborAppIconProvider(this))
{
    ...
    m_pairingPollTimer.setInterval(3000);
    connect(&m_pairingPollTimer, &QTimer::timeout, this, [this] {
        if (m_coreReady
            && (m_pairingPhase == QStringLiteral("IDLE")
                || (m_pairingPhase == QStringLiteral("ERROR")
                    && m_pairingErrorKey == QStringLiteral("error.server.unavailable")))) {
            pairPollIncoming();
        }
    });
    connect(this, &HarborFacade::requestFailed, this,
            [this](const QString &requestType, const QString &) {
                if (requestType == QStringLiteral("pairing.incoming"))
                    m_pairingIncomingPollPending = false;
            });
...
void HarborFacade::pairInvite(const QString &harborId)
{
    if (!isValidHarborId(harborId))
        return;
    requestPairing(QStringLiteral("pairing.invite"),
                   QJsonObject{{QStringLiteral("peer"), harborId}});
}

bool HarborFacade::isValidHarborId(const QString &harborId)
{
    if (harborId.size() != 15 || !harborId.startsWith(QStringLiteral("harbor-")))
        return false;
    for (int index = 7; index < harborId.size(); ++index) {
        const QChar character = harborId.at(index);
...
{
    if (!m_coreReady || m_pairingIncomingPollPending)
        return;
    m_pairingIncomingPollPending = true;
    sendRequest(QStringLiteral("pairing.incoming"), QJsonObject{}, [this](const QJsonObject &reply) {
        m_pairingIncomingPollPending = false;
        applyPairingState(reply);
    });
}
...
void HarborFacade::migrateServerEndpoint(const QString &toAddress)
{
    if (!m_coreReady)
        return;
    QJsonObject payload;
    // Empty uses the core's compiled canary default; explicit values (canary
    // IP, then production hostname) migrate to exactly that destination.
    if (!toAddress.trimmed().isEmpty()) {
        payload.insert(QStringLiteral("to_address"), toAddress.trimmed());
        payload.insert(QStringLiteral("address"), toAddress.trimmed());
    }
    sendRequest(QStringLiteral("server.migrate"), payload,
                [this](const QJsonObject &reply) { applyServerConfig(reply); });
}
...
void HarborFacade::applyNetworkDiagnostics(const QJsonObject &payload)
{
    ...
    const QJsonObject udp = payload.value(QStringLiteral("udp")).toObject();
    const QJsonObject relay = payload.value(QStringLiteral("relay")).toObject();
    ...
    // Phase 4.0: the relay/TURN datagram path (same server socket) and the
    // client-side relay fallback state. Measured or absent — never inferred.
    diagnostics.insert(QStringLiteral("udpReachable"),
                       udp.value(QStringLiteral("reachable")).toBool());
    if (udp.value(QStringLiteral("rtt_ms")).isDouble())
        diagnostics.insert(QStringLiteral("udpRttMs"),
                           udp.value(QStringLiteral("rtt_ms")).toDouble());
    diagnostics.insert(QStringLiteral("relayActive"),
                       relay.value(QStringLiteral("active")).toBool());
...
QString HarborFacade::pairingPeerHarborId() const
{
    return m_pairingPeerHarborId;
}
...
bool HarborFacade::serverNeedsMigration() const
{
    return m_serverNeedsMigration;
}
...
void HarborFacade::applyServerConfig(const QJsonObject &payload)
{
    ...
    const bool needsMigration =
        payload.value(QStringLiteral("needs_migration")).toBool(m_serverNeedsMigration);
    if (m_serverConfigured == configured && m_serverAddress == address
        && m_serverFingerprint == fingerprint && m_serverNeedsMigration == needsMigration)
        return;
    m_serverConfigured = configured;
    m_serverAddress = address;
    m_serverFingerprint = fingerprint;
    m_serverNeedsMigration = needsMigration;
    emit serverChanged();
...
void HarborFacade::applyPairingState(const QJsonObject &payload)
{
    ...
    const QString peerHarborId =
        payload.value(QStringLiteral("peer_harbor_id")).toString(m_pairingPeerHarborId);
    ...
    if (payload.contains(QStringLiteral("request"))) {
        ...
        // Prefer the Harbor ID the server attaches; fall back to the raw
        // requester UUID older payloads carry.
        const QString requesterHarborId =
            request.value(QStringLiteral("requester_harbor_id")).toString();
        incoming.insert(QStringLiteral("name"),
                        request.value(QStringLiteral("requester")).toString());
        if (!requesterHarborId.isEmpty())
            incoming.insert(QStringLiteral("harborId"), requesterHarborId);
...
void HarborFacade::setCoreReady(bool ready)
{
    ...
    if (!ready) {
        ...
        if (m_networkDiagnosticsRunning) {
            m_networkDiagnosticsRunning = false;
            emit networkDiagnosticsChanged();
        }
    }
                               ...
                               std::function<void(const QJsonObject &)> onPayload)
{
    ...
    if (!m_supervisor->sendEnvelope(envelope)) {
        ...
        if (requestType == QStringLiteral("network.diagnostics")
                && m_networkDiagnosticsRunning) {
            m_networkDiagnosticsRunning = false;
            emit networkDiagnosticsChanged();
        }
...
void HarborFacade::handleEnvelope(const QJsonObject &envelope)
{
    ...
    if (!replyTo.isEmpty() && m_pending.contains(replyTo)) {
        ...
        if (envelope.contains(QStringLiteral("error"))) {
            ...
            if (type == QStringLiteral("network.diagnostics")
                    && m_networkDiagnosticsRunning) {
                m_networkDiagnosticsRunning = false;
                emit networkDiagnosticsChanged();
            }
