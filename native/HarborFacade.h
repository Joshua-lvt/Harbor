...
class HarborFacade final : public QObject
{
    ...
    Q_PROPERTY(QString pairingPeerHarborId READ pairingPeerHarborId NOTIFY pairingChanged FINAL)
    Q_PROPERTY(QString pairingErrorKey READ pairingErrorKey NOTIFY pairingChanged FINAL)
    /// The most recent incoming pairing request: {name, harborId?, pairingId} or empty.
    ...
    /// True when the stored pin is an exact legacy K11 endpoint (core
    /// `server.config` `needs_migration`). The UI shows a one-tap migration
    /// banner; migration itself is `migrateServerEndpoint`.
    Q_PROPERTY(bool serverNeedsMigration READ serverNeedsMigration NOTIFY serverChanged FINAL)
...
public:
    ...
    QString pairingPeerHarborId() const;
    ...
    bool serverNeedsMigration() const;
    ...
    /// Idempotent K11 -> Oracle migration: only an exact legacy endpoint
    /// moves, the fingerprint is preserved, custom endpoints are kept. After
    /// a `migrated=true` reply the caller must reconnect (the core swaps
    /// durable state only; live signaling re-pins on its next tick, never
    /// writing to two authorities). Empty `toAddress` uses the core's
    /// compiled canary default.
    Q_INVOKABLE void migrateServerEndpoint(const QString &toAddress = QString());
    /// Points a fresh install at the Harbor network (the server's public
    /// IPv6 endpoint, same endpoint the mobile client ships pre-pointed
    /// at) when no server pin exists yet. One shot per core session; an
    /// existing pin is never overwritten. The shell calls this on
    /// coreReady and serverChanged; the facade itself knows whether
    /// server.config has been fetched yet, which QML cannot observe.
    ...
    /// Legacy flow, kept for older clients. New UI invites by Harbor ID.
    ...
    /// Harbor-ID invitation: invite the device behind `harborId`
    /// (`harbor-xxxxxxxx`) and wait for its approval. This is the flow the
    /// current UI uses; the code flow above stays for compatibility.
    Q_INVOKABLE void pairInvite(const QString &harborId);
    /// Exact Harbor-ID grammar used before an invite can reach the core.
    static bool isValidHarborId(const QString &harborId);
...
private:
    ...
    QTimer m_pairingPollTimer;
    bool m_pairingIncomingPollPending = false;
    bool m_serverConfigured = false;
    QString m_serverAddress;
    QString m_serverFingerprint;
    bool m_serverNeedsMigration = false;
